...
({
	...
	buildColumns : function(component, templates, listHeader) {
		...
		for (var row = 0; row < columnTemplateRows.length; row++) {
			...
			// Loop over columns for the current header row. These are columns
			// that might be rowSpanned into cross-index positions
			for (var column = 0; column < columnCount; column++) {
				// We need to work with the concrete component to get all of the
				// concrete attributes
				...
				// Generate actual column header cell and preserve the reference
				// for the flattened v.columns and v.dataColumns attributes
				...
				// Copy template events now to prevent iterating in the build
				// item loop
				template.events = this.getEvents(template);
			}
			var headerRow = this.generateComponent("ui:listViewRow", {
				body : columns,
				"class" : rowClass
			});
			headerRows.push(headerRow);
		}
		// Add the columns to the header, save the processed columns and fire
		// the column ready event
	...
	buildRows : function(component, templates, listBody) {
		...
		function buildEmptyMessage(helper) {
			...
			var emptyCell = helper.generateComponent("aura:html", {
				body : [ emptyMessage ],
				tag : "TD",
				HTMLAttributes : $A.expressionService.create(component, {
					"class" : "emptyMessage",
					colSpan : dataTemplateCount || 1
				})
			});
			var row = helper.generateComponent("ui:listViewRow", {
				body : [ emptyCell ],
				"class" : helper.getRowClass(component, 0)
			});
		...
		function buildRowBlock() {
			...
			if (index && !containerSet) {
				// Set a rough container height estimate based on the first row
				// of data
			...
			for (var i = 0; i < blockSize && index < itemCount; i++, index++) {
				...
				var row = this.generateComponent("ui:listViewRow", {
					body : columns,
					"class" : rowClass,
					tooltip : rowTooltip,
					item : item,
	...
	fireEvents : function(component, type, domEvent, eventParams) {
		...
		case "Header":
			...
			// Intentional fall-through to also fire row events for cells and
			// headers. Thus, no break statement.
		case "Row":
	...
	getColumnTemplates : function(component, listId, columnTemplates, dataTemplates, colSpans, headers, depth, nextId) {
		...
		if (listViewColumns.length > 0) {
			...
			for (var i = 0; i < listViewColumns.length; i++) {
				// We have to work with the concrete component to get access to
				// all attributes
				...
				// Generate header ids and add them to columns and accessibility
				// headers
				id = [ listId, "header", depth, nextId++ ].join(':');
				column.set("v.id", id);
				headers.push(id);

				// Look for nested column templates recursively, then add our
				// columns to the row
				...
				// And update the column span with the current count of the
	...
	getEventTargets : function(component, target) {
		...
		targets.column = this.getTarget(target, listViewElement, {
			TH : 1,
			TD : 1
		});
		...
		targets.section = this.getTarget(targets.row, listViewElement, {
			THEAD : 1,
			TBODY : 1,
			TFOOT : 1
		});
	...
	getTextContent : function(facet) {
		// Crude recursive attempt to build literal strings from the lowest
		// level components
		var text = '';
		if (facet) {
	...
	setImmediate : function(callback) {
		...
		this.setImmediate = (global.requestAnimationFrame || global.webkitRequestAnimationFrame || global.mozRequestAnimationFrame
				|| global.oRequestAnimationFrame || global.msRequestAnimationFrame || function(callback) {
