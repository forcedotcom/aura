function tester() {
    ...
    // Test all properties along prototyoe hierachy to detect
    // missing properties.
    ...
    // in object prototype hierachy between system and locker.
    function testSecure(object) {
        ...
        // Add all extra properies under "Object"
    ...
    function executeTests(object, proto, prop, source) {
        ...
        } else if (source === "locker" && plan && plan.type) {
            type.status = (type.value === toLockerType(plan.type) ? 'pass' : 'fail');
        }
    ...
    function addResults(proto, prop, source, results) {
        ...
        if (!testResults[proto]) { testResults[proto] = {}; }
        if (!testResults[proto][prop]) { testResults[proto][prop] = {}; }
    ...
    // Returm all tests results by property and by prototype.
    function makeReport() {
        // Pivot the data
        var report = {
