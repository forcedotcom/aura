({
    ...
    testAttemptToEvalToWindow: function(cmp, event, helper) {
        var testUtils = cmp.get("v.testUtils");
        ...
        testUtils.assertEquals('function(){}', helper.removeWhitespaces((function() { return ({}).constructor.constructor('return this') + ''; })() + '', 'Expected realm function constructor to silently fail.'));

        // Check defense against malicious function body
        try {
            window.test = 1;
            new Function('}, window.test = 2, function dummy(){');
            testUtils.fail("Expected invalid Function body to throw.");
        } catch (e) {
            testUtils.assertEquals(1, window.test, 'Expected Function created witout side-effect.');
            testUtils.assertStartsWith("SyntaxError", e.toString(), "Unexpected error. Expected SyntaxError, got " + e);
        }
