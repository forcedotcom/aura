<aura:application render="client" model="java://org.auraframework.components.test.java.model.TestIterationModel" 
    ...
    <miscTest:htmlMarkup color="{!v.color}"/>
    <br/>
    ...
    <ui:button label="Cut the team by half" press="{!c.by2}" class="changeIteratonIndex"/>
	    <table >
		    <tr>
			    <miscTest:iterateComponents color="{!v.color}" start="{!v.start}" end="{!v.end}">
			    ...
			    </miscTest:iterateComponents>    
		    </tr>
		    </table>
		    <table>
		    <tr>
			    <td>
			    <miscTest:iterateBasicData color="{!v.color}" rowNumber="{!v.count}" start="{!v.start}" end="{!v.end}"/>
			    </td>
			    <td >
			    <miscTest:deepIteration color="{!v.color}" players="{!m.playerData}" start="{!v.start}" end="{!v.end}"/>
			    </td>
		    </tr>
	    </table>
	...
	<miscTest:inheritance color="{!v.color}"/>
