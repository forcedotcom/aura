...
({
	 ...
	 * Test Flow:
	 *
	 * 1. Click the button with the controller that sets v.isDomEventSet
	 *    when the button triggers a domEvent
	 * 2. Verify that v.isDomEventSet is set
	 *
	 */
	testDomEventAttributeOnClick : {
		test : [function(cmp) {
			var btnElm = cmp.find("testDomEventBtn").getElement();
			btnElm.click();
		}, function(cmp) {
			$A.test.assertTrue(cmp.get("v.isDomEventSet"),
					"domEvent attribute on event should have been set");
		}]
	},
