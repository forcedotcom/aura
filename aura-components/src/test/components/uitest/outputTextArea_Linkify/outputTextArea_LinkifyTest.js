...
({
    ...
    testImgInText:{
        ...
        test: function(cmp){
            this.assertLinksPresent(cmp, "href=\"http://www2.sfdcstatic.com/common/assets/img/logo-tag-company.png\"", false);
            this.assertLinksPresent(cmp, "href=\"http://www.salesforce.com\"", false);
            // make sure the href in the link isn't linkified
    ...
    assertLinksPresent: function(cmp, hrefText, checkValue) {
        $A.test.addWaitForWithFailureMessage(true,
            function() {
                var htmlValue = cmp.find("textAreaComp").getElement().innerHTML;
                return $A.test.contains(htmlValue, hrefText);
            }, "couldn't find " + hrefText + " in: "  + cmp.find("textAreaComp").getElement().innerHTML,
            function() {
                // The following assertion wouldn't work if the text contains html elements
                if (checkValue) {
                    var textValue = $A.test.getText(cmp.find("textAreaComp").getElement());
    ...
    assertTextNotPresent: function(cmp, text) {
        $A.test.addWaitForWithFailureMessage(true,
            ...
            }, "Found " + text + " in: "  + cmp.find("textAreaComp").getElement().innerHTML)
    }
