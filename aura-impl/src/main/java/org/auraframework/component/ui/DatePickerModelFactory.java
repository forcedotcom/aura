...
public class DatePickerModelFactory implements ModelFactory<DatePickerModel> {

    @Inject
    private LocalizationAdapter localizationAdapter;

    @Override
    public DatePickerModel modelInstance() throws ModelInitializationException {
        try {
            return new DatePickerModel(localizationAdapter);
        } catch (Exception x) {
