...
public class JavaControllerDefFactory extends BaseJavaDefFactory<ControllerDef> {
    ...
    protected DefBuilder<?, ? extends ControllerDef> getBuilder(DefDescriptor<ControllerDef> descriptor) throws QuickFixException {
        ...
        if(!c.isAnnotationPresent(Controller.class)){
            throw new InvalidDefinitionException(String.format("@Controller annotation is required on all Controllers.  Not found on %s", descriptor), builder.getLocation());
        }
    ...
    private static JavaActionDef makeActionDef(Method method, Class<?> controllerClass,
        ...
        if (!Modifier.isPublic(modifiers) || !Modifier.isStatic(modifiers)) {
            // We used to just ignore this, but it is really bad, as someone marked an invalid routine
            // as AuraEnabled
            throw new InvalidDefinitionException("Actions must be public static methods",
        ...
        actionBuilder.setReturnTypeDescriptor(DefDescriptorImpl.getInstance("java://"+method.getReturnType().getName(), TypeDef.class));
        ...
        for (int i = 0; i < paramTypes.length; i++) {
            ...
            for(Annotation annotation : annotations[i]){
                if(annotation instanceof Key) {
                    ...
                    ValueDef valueDef = new JavaValueDef(((Key)annotation).value(), typeDefDesc,
                                                         new Location(controllerClass.getName()+"."+name,0));
                    params.add(valueDef);
                }
            }
            ...
            if (method.isAnnotationPresent(AuraEnabled.class)) {
                ...
                if (action != null) {
                    ...
                    if (actions.containsKey(action.getName())) {
                        throw new InvalidDefinitionException("Duplicate action "+action.getName(),
                                                             new Location(controllerClass.getName(),0));
                    }
