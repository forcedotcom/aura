...
public final class ComponentImpl extends BaseComponentImpl<ComponentDef, Component> implements Component {
    ...
    public ComponentImpl(DefDescriptor<ComponentDef> descriptor, Map<String, Object> attributes) throws QuickFixException{
        super(descriptor, attributes);
    }

    public ComponentImpl(DefDescriptor<ComponentDef> descriptor, Collection<AttributeDefRef> attributeDefRefs, BaseComponent<?,?> attributeValueProvider, String localId) throws QuickFixException{
        super(descriptor, attributeDefRefs, attributeValueProvider, localId);
    }

    public ComponentImpl(DefDescriptor<ComponentDef> descriptor, Collection<AttributeDefRef> attributeDefRefs, BaseComponent<?,?> attributeValueProvider, Map<String, Object> valueProviders, ValueProvider delegateValueProvider) throws QuickFixException{
        super(descriptor, attributeDefRefs, attributeValueProvider, valueProviders, delegateValueProvider);
    }

    protected ComponentImpl(DefDescriptor<ComponentDef> descriptor, Component extender, BaseComponent<?,?> attributeValueProvider, Component concreteComponent) throws QuickFixException{
    ...
    protected void injectComponent() throws QuickFixException {
        if (this.intfDescriptor != null){
            ...
            BaseComponent<?,?> oldComponent = context.setCurrentComponent(new ProtoComponentImpl(descriptor, getGlobalId(), attributeSet));
            try {
                RootDefinition root = intfDescriptor.getDef();
                ProviderDef providerDef = root.getLocalProviderDef();
                if (providerDef == null){
                    throw new InvalidDefinitionException(String.format("%s cannot be instantiated directly.", descriptor), root.getLocation());
                }
                    ...
                    if (config != null) {
                        ...
                        try {
                            if (descriptor.getDefType() != DefType.COMPONENT) {
                                throw new AuraRuntimeException(String.format("%s did not provide a valid component", providerDef.getDescriptor()));
                            }

                            ComponentDef c = descriptor.getDef();
                            if (c.isAbstract()) {
                                throw new AuraRuntimeException(String.format("%s cannot be instantiated directly.", descriptor));
                            }
                        ...
                        if (providedAttributes != null) {
                            // if there is a remote provider and attributes were set, we assume/pray the remote provider does too
