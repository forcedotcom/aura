...
/**
 * Implementation of Parser. Parses XML Formatted Source to produce ComponentDefs. Implemented as a pull-style parser
 * using the StAX cursor API to try to keep the memory footprint low, and reduce creation of extraneous Objects.
 */
public class XMLParser implements Parser {
    ...
    public <D extends Definition> D parse(DefDescriptor<D> descriptor, Source<?> source) throws QuickFixException {
        ...
        try {
            ...
            LOOP : while (xmlReader.hasNext()) {
                ...
                switch(type){
                    ...
                    case XMLStreamConstants.SPACE:
                        break;
                    default:
                        throw new InvalidDefinitionException(String.format("Found unexpected element of type %s", type), getLocation(xmlReader, source));
                }
            }
            if (!xmlReader.hasNext()) {
                throw new InvalidDefinitionException("Empty file", getLocation(xmlReader, source));
            }
            ret = (D)RootTagHandlerFactory.newInstance((DefDescriptor<RootDefinition>)descriptor, (Source<RootDefinition>)source, xmlReader).getElement();

            // the handler will stop at the END_ELEMENT, verify there is nothing left
            ...
            LOOP : while (xmlReader.hasNext()) {
                ...
                switch(type){
                    ...
                    case XMLStreamConstants.SPACE:
                        break;
                    default:
                        throw new InvalidDefinitionException(String.format("Found unexpected element of type %s when expecting end of file.", type), getLocation(xmlReader, source));
                }
            }
            ...
            try {
                ...
                // Throwing this seems wrong, if there was already an error, it should pass through,
                // and if not, well, something went wrong with the close...
                //throw new AuraUnhandledException("parse error", getLocation(xmlReader, source), e);
            } finally {
                try {
                    if (xmlReader != null) {
                        xmlReader.close();
    ...
    /**
     * Returns a location for the reader and source provided.  When {@code xmlReader}
     * is provided, its location will be used for the finer-grain information such as
     * line number; otherwise, a new and more limited location will be constructed
     * based on {@code source} and the system ID therein.
     *
     * @param xmlReader
     * @param source
    ...
    public static Location getLocation(XMLStreamReader xmlReader, Source<?> source) {
        if (xmlReader != null) {
            ...
            // prefer source.getUrl() over xmlReader.getLocation().getSystemId().
            ...
            if (location == null) {
                // This will happen for external subclasses of Source, but we can't
