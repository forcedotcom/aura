...
public class ForEachDefHandler<P extends RootDefinition> extends ParentedTagHandler<ForEachDef, P> implements ExpressionContainerHandler {
    ...
    private ForEachDefImpl.Builder builder = new ForEachDefImpl.Builder();
    ...
    protected ForEachDef createDefinition() {
        ...
        SubDefDescriptor<ComponentDef, ? extends BaseComponentDef> subDefDescriptor = parentHandler.createSubComponentDefDescriptor(TAG);
        componentBuilder.setDescriptor(subDefDescriptor);

        DefDescriptor<AttributeDef> varAttributeDescriptor = DefDescriptorImpl.getInstance(builder.getVar(), AttributeDef.class);
        DefDescriptor<TypeDef> varTypeDescriptor = DefDescriptorImpl.getInstance("String", TypeDef.class); //FIXME
        AttributeDef varAttributeDef = new AttributeDefImpl(varAttributeDescriptor, componentBuilder.getDescriptor(), varTypeDescriptor, null, true, AttributeDef.SerializeToType.BOTH, getLocation());
    ...
    protected void readAttributes() throws QuickFixException {
        Expression e = AuraImpl.getExpressionAdapter().buildExpression(TextTokenizer.unwrap(getAttributeValue(ATTRIBUTE_ITEMS)), getLocation());
