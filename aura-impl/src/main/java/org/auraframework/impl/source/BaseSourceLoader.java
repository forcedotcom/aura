...
public abstract class BaseSourceLoader implements SourceLoader {
    ...
    public enum NameFormat {
        ...
        JAVA,               //! A java style file (x.y.z.Name$Inner)
    ...
    protected String getPath(DefDescriptor<?> descriptor) {
        ...
        case BUNDLED_EXTRA:
            ...
            return String.format("%s/%s/%s%s", namespace, bundle.getName(), name, ei.extension);
        case BUNDLE:
            if (bundle != null) {
                // whoops.
                throw new AuraRuntimeException("Invalid "+descriptor+"@"+descriptor.getDefType()+" with ei="+ei);
            ...
            return String.format("%s/%s/%s%s", namespace, name, name, ei.extension);
        case JAVA:
        ...
        case NAMESPACE:
            return String.format("%s/%s%s", namespace, namespace, ei.extension);
        }
        throw new AuraRuntimeException("Could not get path for "+descriptor+"@"+descriptor.getDefType()+" with ei="+ei);
    }
    ...
    public static boolean isValidNameForDefType(DefType defType, String path) {
        DefDescriptor<?> dd = getDescriptor(path);
        return dd != null && dd.getDefType() == defType;
    }
}
