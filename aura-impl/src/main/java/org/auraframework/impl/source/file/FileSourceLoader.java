...
public class FileSourceLoader extends BaseSourceLoader {
    ...
    //Tests create loaders like crazy, which takes time to scan for namespaces, so this caches that mapping.
    ...
    public FileSourceLoader(File base) {
        ...
        if (base == null || !base.exists() || !base.isDirectory()) {
            throw new AuraRuntimeException(String.format("Base directory %s does not exist", base == null ? "null" : base
                    .getAbsolutePath()));
        }
        this.base = base;
    }
    ...
    public <D extends Definition> FileSource<D> getSource(DefDescriptor<D> descriptor) {
        ...
        if(!isFilePresent(file)){
            ...
            if(file.exists()){
                descriptor = updateDescriptorName(descriptor, file.getParentFile().getParentFile().getName(), file.getName());
            }
        }
     ...
     *
     * This will recursively walk a set of directories to find all files that matche the filter, in any
     * directory.
     *
     ...
     *
     * We need to do this because we don't know a-priory what the types are, and rather than redo
    ...
    private static class AnyTypeFilter implements FileFilter {
         ...
         *
         * This must be called before this is used as a filter, otherwise it will
