...
public class BrowserUserAgent {
     ...
     */
    private static final String mobileRegexp1 = ".*(android.+mobile|apexmobile|avantgo|bada\\/|blackberry|bb10|blazer|compal|elaine|" +
    ...
    public static int parseBrowser(String userAgent) {
        ...
        // return the resulting number - this is what goes in the DB for browser 
        // type
        return browser;
    }

    /**
     * Parse user agent string to determine platform.
     *
    ...
    public static int parsePlatform(String userAgent) {
        ...
        if (ua.contains(ANDROID)) {
            try {
                ...
                if (idx != -1) {
                    idx = idx + 15; // 15 = "android mobile/".length()
                } else {
                    // usually it is "android 1.234"
                    idx = ua.indexOf(ANDROID) + 8; // 8 = "android ".length()
                }
        ...
        if (ua.contains(WIN) && !ua.contains(DARWIN)) {
            if (ua.contains(PHONE)) {
                ...
                } else if (ua.contains(WINDOWS_PHONE_8)) {
            ...
            } else if (ua.contains(WIN_8_1)) {
                if (ua.contains(ARM)) {
                    return PLATFORM_WIN_RT_8_1;
                } else {
