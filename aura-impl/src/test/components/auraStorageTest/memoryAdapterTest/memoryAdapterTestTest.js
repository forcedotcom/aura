...
({
	...
	test : [function(cmp) {
		    ...
		    $A.test.assertEquals("key1,key2", adapter.getMRU()
			    .toString());
		},
		function(cmp) {
		    var adapter = this.adapter;
			...
			$A.test.assertEquals("key2,key1", adapter.getMRU()
				.toString());
			...
			$A.test.addWaitFor(true, function() {
			    ...
			    $A.test.assertEquals("key1,key3", adapter.getMRU()
				    .toString());
			});
		    });
		},
