...
({
    ...
    testTypesCscc:{
        test:function(cmp){
            ...
            // strings
            container = cmp.find("stringsCscc");
            containerElement = container.getElement();
            children = $A.test.getNonCommentNodes(containerElement.childNodes);
            $A.test.assertEquals(3, container.get("v.body")[0].get("v.realbody").length);
            $A.test.assertEquals(3, children.length);
            ...
            // numbers
            container = cmp.find("numbersCscc");
            containerElement = container.getElement();
            children = $A.test.getNonCommentNodes(containerElement.childNodes);
            $A.test.assertEquals(3, container.get("v.body")[0].get("v.realbody").length);
            $A.test.assertEquals(3, children.length);
            ...
            // booleans
            container = cmp.find("booleansCscc");
            containerElement = container.getElement();
            children = $A.test.getNonCommentNodes(containerElement.childNodes);
            $A.test.assertEquals(3, container.get("v.body")[0].get("v.realbody").length);
            $A.test.assertEquals(3, children.length);
            ...
            // barecomponents
            // container = cmp.find("barecomponentsCscc");
            // containerElement = container.getElement();
            // children = $A.test.getNonCommentNodes(containerElement.childNodes);
            // $A.test.assertEquals(3,
            // container.get("v.body")[0].get("v.realbody").length);
    ...
    //Aura.Component[] as items
    testIterationThroughAuraComponentArrayCscc:{
        test:function(cmp){
            var container = cmp.find("componentArrayCscc");
            var elements = $A.test.getNonCommentNodes(container.getElements());
            $A.test.assertEquals(2, elements.length);
            //Verify that iteration component's body has only two components.
            $A.test.assertEquals(2, container.find("iterationCscc").get("v.realbody").length);
    ...
    //Aura.Component[] as items
    testIterationThroughEmptyComponentArrayCscc:{
        test:function(cmp){
            //Aura.Component[] as items
            var container = cmp.find("emptyComponentArrayCscc");
            var elements = $A.test.getNonCommentNodes(container.getElements());
            $A.test.assertEquals(0, elements.length);
            $A.test.assertEquals(0, container.find("iterationCscc").get("v.realbody").length);
        }
    }
})
