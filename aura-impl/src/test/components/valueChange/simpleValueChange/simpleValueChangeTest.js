...
({
    ...
    /*
     * X observe Y : X is watching Y, which means Y.$observers$ has X, and X.$observing$ is Y
     */
    testObservers : {
        test:function(cmp) { //sanity test
            ...
            /* X:attrib  Y:value 
             * case1.1: 
             *         1.1.1: make X observe Y 
             *         1.1.2: X's old value will be replaced with Y's value
            */ 
            ...
            /* X:attrib  Y:value  X observe Y
             * case1.2: when Y's value change, X's value get updated.
             */
            ...
            /* X:attrib Y:value X observe Y
             * case1.3: try to Z that X is NOT observing from X observing, should result in warning message on console
             * case1.4: 
             *         1.4.1: call X.observe(Y) again, not necessary, as X is already observing Y.
             *         1.4.2: change Y's value, X's value get udpated
             */
            ...
            /*
             * X:attrib Y:value X observe Y
             * case 1.5: drop X's observing (Y). change Y's value, X's value stay the same
             */
            ...
            /*
             * X:attrib Y:value. X is not observing after case1.5
             * case1.6: 
             *         1.6.1: make Y observe X
             *         1.6.2: change X's value, Y's value get updated.
             */
        ...
        test:function(cmp) { //let's try some chain, and also loops
            ...
            /*
             * X:attrib A:valueA. 
             * case 2.1: loop, two value try to watch each other. 
             * same thing will happen if the loop contains more than two value
             *          2.1.1: set both A and X to "six"
             *          2.1.2: make X observe A , A observe X
             *          2.1.3: change A's value, 
             *          first, A will stop observing X since X's value is different than A's, but X still watch A, so X get updated
             */
            ...
            /*
             * X:attrib A:valueA B:valueB C:valueC  
             * case 2.2: one value is watched by multiple value
             *          2.2.0: init X,A,B,C to different value
             *          2.2.1: make A observe X, B observe X, C observe X
             *          2.2.2: change X's value, A,B and C should get updated
             */
            ...
            /*
             * X:attrib A:valueA B:valueB C:valueC 
             * case 2.3: chained watch
             *           2.3.1: init X,A,B,C to different value
             *           2.3.2: make A observe B, B observe C, C observe X
             *           2.3.3: change X's value, A,B and C should get updated
             */
            ...
            /*
             * X:attrib A:valueA B:valueB C:valueC 
             * case 2.4: one value try to watch more than one value, the last one win 
             *          2.4.1: init X,A,B,C to "nine"
             *          2.4.2: make X observe A, X observe B, X observe C, X will end up watching C only
             *          2.4.3: change A's value, X won't change
             *          2.4.4: change C's value, X get updated
             */
