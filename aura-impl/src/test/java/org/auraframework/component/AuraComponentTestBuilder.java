 ...
 */
package org.auraframework.component;
 ...
 *
 * This class can be used to build and auto-clean up a test directory with a components directory
 * nested inside. It can then be inserted in the set of loaders using {@link #installComponentLocationAdapter()}.
 * it implements {@link AutoCloseable} with an idempotent close method so that it can be called multiple times.
 * In the event that tests are run in a try-with-resources, this should automatically get cleaned up.
 */
public class AuraComponentTestBuilder extends DescriptorFileMapper implements AutoCloseable {
    ...
    private ComponentLocationAdapter cla;
     ...
     *
     * This function puts the component location adapter in the set that is returned by the
     * Service loader. It uses the mock, so one must be careful with it.
     */
     ...
     *
     * This allows building a new bundle without caring about the name. It builds it in a new
     * namespace.
     *
     ...
     *
     * This should be used for the bundle-defining name, and subsequent calls should use the descriptor form
     * since this will create a new name.
     */
