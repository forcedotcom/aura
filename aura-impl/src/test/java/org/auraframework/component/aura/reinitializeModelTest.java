...
public class reinitializeModelTest extends AuraImplTestCase {
    ...
    private DefDescriptor<ComponentDef> getDefDescriptorExtend(String extend, String attname, String attvalue,String extraAttr, String mvalue, String extraCmp) {
        boolean debug=false;
        String extraAttribute="";
        if( extraAttr!=null && extraAttr.length()>0) { extraAttribute=extraAttr; }
        String extraComponent="";
        if( extraCmp!=null && extraCmp.length()>0) { extraComponent=extraCmp; }
        String source = String.format(componentTagExtendWithAttr,extend,attname,attvalue,extraAttribute,mvalue,extraComponent);
        DefDescriptor<ComponentDef> def = addSourceAutoCleanup(ComponentDef.class,source);
        if(debug) System.out.println(def.getDescriptorName()+":\n"+source);
        return def;
    }
    
    private DefDescriptor<ComponentDef> getDefDescriptor(String tag, String model, String attr, String extraAttr, String extraCmp) {
        boolean debug = false;
        String attribute = "defaultAttribute";
        if(attr!=null && attr.length()>0) { attribute=attr;} 
        String extraAttribute="";
        if( extraAttr!=null && extraAttr.length()>0) { extraAttribute=extraAttr; }
        String extraComponent="";
        if( extraCmp!=null && extraCmp.length()>0) { extraComponent=extraCmp; }
    ...
    public void runTestReinitializedModelSuperCmp(Boolean testModelWithCount, String model) throws Exception {
        if(testModelWithCount) { TestReinitializeModelWithCount.clearCount(); }
        Map<String, Object> attributes = Maps.newHashMap();
        
        DefDescriptor<ComponentDef> defDesParent = getDefDescriptor(componentTagWithModelAndAttr,model,"'defaultattributeinParent1'","","");

        DefDescriptor<ComponentDef> defDesGrandChild = getDefDescriptorExtend
        (defDesParent.getDescriptorName(),"attributeInGrandChild","default attribute in GrandChild","","{!v.attributeInGrandChild}","");
        String cmpGrandChild = "<"+defDesGrandChild.getDescriptorName()+" attributeInGrandChild='{!v.attributeFromChildToGrandChild}'"+"/>";
        
        String attributeFromChildToGrandChild = String.format(attributeTag,"'attributeFromChildToGrandChild'","'String'","'default attributeFromChildToGrandChild'");
        DefDescriptor<ComponentDef> defDesChild = getDefDescriptorExtend
        (defDesParent.getDescriptorName(),"attributeInChild","default attribute in Child",attributeFromChildToGrandChild,"{!v.attributeInChild}",cmpGrandChild);
    ...
    public void runTestSameCmpSameModel(Boolean testModelWithCount, String model) throws Exception {
        if(testModelWithCount) { TestReinitializeModelWithCount.clearCount(); }
        DefDescriptor<ComponentDef> defDes1 = getDefDescriptor(componentTagWithModelAndAttr,model,"'defaultattributeinParent1'","","");
        String source1 = "<"+defDes1.getDescriptorName()+" attrInParent='{!v.attributeInMain1}'"+"/>";
        DefDescriptor<ComponentDef> defDes2 = getDefDescriptor(componentTagWithModelAndAttr,model,"'defaultattributeinParent1'","","");
        String source2 = "<"+defDes2.getDescriptorName()+" attrInParent='{!v.attributeInMain2}'"+"/>";
        DefDescriptor<ComponentDef> defDes3 = getDefDescriptor(componentTagWithModelAndAttr,model,"'defaultattributeinParent1'","","");
     ...
     * they are different component, but use same model
     * Test:
     * change attribute value (attributeInMain[,1,2,3]), then reinitializeModel on main component
     */
    public void runTestDiffCMPSameModel(Boolean testModelWithCount, String model) throws Exception {
        ...
        DefDescriptor<ComponentDef> defDes2 = getDefDescriptor(componentTagIfWithModelAndAttr,model,"'defaultattributeinParent2'","","");
     ...
     * 
     * bug filed W-2107437 :
     * funny thing if we accidently include a component itself in its markup, we will end up with infinity loop then out of memory.
     */
    public void runTestNestedCmp (Boolean testModelWithCount, String model) throws Exception {
        if(testModelWithCount) { TestReinitializeModelWithCount.clearCount(); }
        Map<String, Object> attributes = Maps.newHashMap();
        
        DefDescriptor<ComponentDef> defDes3 = getDefDescriptor(componentTagWithModelAndAttr,model,"'defaultattributeinParent3'","","");
        String cmpString3 = "<"+defDes3.getDescriptorName()+" attrInParent='{!v.attributeInParentContainer}'"+"/>";
        
        String att2to3 = String.format(attributeTag,"'attributeInParentContainer'","'String'","'default attributeInParentContainer'");
        
        DefDescriptor<ComponentDef> defDes2 = getDefDescriptor(componentTagWithModelAndAttr,model,"'defaultattributeinParent2'",att2to3,cmpString3);
        ...
        String att1to2 = String.format(attributeTag,"'attMainToParentContainer'","'String'","'defaultattribute1to2'");
        String att1to3 = String.format(attributeTag,"'attMainToParent'","'String'","'defaultattribute1to3'");
        DefDescriptor<ComponentDef> defDes1 = getDefDescriptor(componentTagWithModelAndAttr,model,"'defaultattributeinParent1'",att1to2+att1to3,cmpString2);
    ...
    public void runTestCmpAsAttribute(Boolean testModelWithCount, String model) throws Exception {
        ...
        DefDescriptor<ComponentDef> defDes = getDefDescriptor(componentTagWithModelAndAttr,model,"'defaultattributeinParent'","","");
        ...
        DefDescriptor<ComponentDef> newDefDes = getDefDescriptor(componentTagWithModelAndAttr,model,"'defaultattributeinParent'","","");
