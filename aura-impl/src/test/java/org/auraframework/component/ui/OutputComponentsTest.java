...
public class OutputComponentsTest extends AuraTestCase {
    ...
    private Set<ComponentDef> getUiOutputComponents() throws Exception {
        ...
        for (DefDescriptor<ComponentDef> def : definitionService.find(matcher)) {
            if (def.getName().startsWith("output")) ret.add(def.getDef());
        }
        return ret;
    }
    ...
    private ComponentDef getUiOutputComponent() throws Exception {
        DefinitionService definitionService = Aura.getDefinitionService();
        ComponentDef def = definitionService.getDefinition("markup://ui:output",
                ComponentDef.class);
        return def;
    }
    ...
    /**
     * Verify that ui:output is not registered to throw certain events like "change" that are part of DOM Events.
     */
