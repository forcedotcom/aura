...
public class ImportDefTest extends AuraImplTestCase {
    ...
    /**
     * Tests to ensure that the imported libraries are properly compiled with the component.
     */
     ...
     */
    public void testNoLibraryAttribute() throws Exception {
        DefDescriptor<ComponentDef> cmpDesc = addSourceAutoCleanup(ComponentDef.class,
                String.format(baseComponentTag, "", "<aura:import property='dummy'/>"));
        try {
            Aura.getDefinitionService().getDefinition(cmpDesc);
            fail("QuickFixException expected. Library attribute is missing.");
        } catch (Throwable t) {
            assertExceptionMessageEndsWith(t, InvalidDefinitionException.class,
                    "Import must specify a valid library name.");
        }
    }
     ...
     */
    public void testNoPropertyAttribute() throws Exception {
        DefDescriptor<ComponentDef> cmpDesc = addSourceAutoCleanup(ComponentDef.class,
        ...
        try {
            ...
            assertExceptionMessage(t, InvalidDefinitionException.class,
                    "aura:import must specify property=\"…\"");
        }
    }

    /**
     * Exception thrown when no properties specified
     */
    public void testNoProperties() throws Exception {
        DefDescriptor<ComponentDef> cmpDesc = addSourceAutoCleanup(ComponentDef.class,
                String.format(baseComponentTag, "", "<aura:import/>"));
        try {
            Aura.getDefinitionService().getDefinition(cmpDesc);
            fail("QuickFixException expected. property attribute is missing.");
        } catch (Throwable t) {
