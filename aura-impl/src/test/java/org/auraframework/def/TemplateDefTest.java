 ...
 * Components can be marked as template using the "isTemplate" attribute. Applications cannot be marked as template.
 * Both applications and components can use templates. By default, "aura:template" is template for all applications
 * and components.
 */
public class TemplateDefTest extends AuraImplTestCase {
    ...
    public void testCustomTemplate() throws Exception{
        DefDescriptor<ComponentDef> template = addSourceAutoCleanup(ComponentDef.class, String.format(baseComponentTag, "", ""));
        assertTemplate(ComponentDef.class,
                String.format(baseComponentTag, String.format("template='%s:%s'",template.getNamespace(),template.getName()), ""),
                template, "Failed to register a custom template for a component"
        );

        assertTemplate(ApplicationDef.class,
                String.format(baseApplicationTag, String.format("template='%s:%s'",template.getNamespace(),template.getName()), ""),
    ...
    public void testApplicationIsNotATemplate(){
        ...
        try{
            ...
            checkExceptionFull(expected, InvalidSystemAttributeException.class, "Invalid attribute isTemplate", null);
        }
    }
    /**
    ...
    public void testIsTemplateAttributeRequired(){
        ...
        cmp = addSourceAutoCleanup(ComponentDef.class,
                String.format(baseComponentTag, String.format("template='%s:%s'",nonTemplate.getNamespace(),nonTemplate.getName()),""));
        try{
            cmp.getDef();
            fail("Should have failed to use a template marked with isTemplate='false'");
        }catch (Exception expected){
            checkExceptionFull(expected, AuraRuntimeException.class, "Non template component specified for template attribute", null);
        }
    }
    /**
    ...
    public void testTemplateCannotBeAbstract(){
        ...
        try{
            ...
            checkExceptionFull(expected, AuraRuntimeException.class, "Template cannot be abstract.", null);
        }
    }
    ...
    public void testTemplateExtension()throws Exception{
        ...
        DefDescriptor<ComponentDef> child = addSourceAutoCleanup(ComponentDef.class,
                String.format(baseComponentTag, String.format("extends='%s:%s'", parent.getNamespace(),parent.getName()),""));
    ...
    public void testValidScriptTags() throws Exception{
        ...
        DefDescriptor<ComponentDef> parent = addSourceAutoCleanup(ComponentDef.class,
                String.format(baseComponentTag,"isTemplate='true' extensible='true'","<aura:attribute name='scriptTags' type='String'/>"));
        DefDescriptor<ComponentDef> scriptTagInAuraSet = addSourceAutoCleanup(ComponentDef.class,
                String.format(baseComponentTag, String.format("isTemplate='true' extends='%s:%s'", parent.getNamespace(),parent.getName()),
    ...
    public void testInvalidScriptTags()throws Exception{
        ...
        DefDescriptor<ComponentDef> parent = addSourceAutoCleanup(ComponentDef.class,
                String.format(baseComponentTag,"extensible='true'","<aura:attribute name='scriptTags' type='String'/>"));
        DefDescriptor<ComponentDef> scriptTagInAuraSet = addSourceAutoCleanup(ComponentDef.class,
                String.format(baseComponentTag, String.format("extends='%s:%s'", parent.getNamespace(),parent.getName()),
                        String.format("<aura:set attribute='scriptTags'>%s</aura:set>",scriptInclude) ));
        assertExceptionDueToScripts("Script tags should not be allowed as attribute value in components", scriptTagInAuraSet, true);
    }
    private void assertExceptionDueToScripts(String msg, DefDescriptor<? extends BaseComponentDef> desc, boolean expectException){
        try{
            desc.getDef();
            if(expectException)  fail(msg);
        }catch(Exception e){
