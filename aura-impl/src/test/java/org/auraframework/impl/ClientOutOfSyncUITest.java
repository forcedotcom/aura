...
public class ClientOutOfSyncUITest extends WebDriverTestCase {
    ...
    private DefDescriptor<ComponentDef> setupTriggerComponent(String attrs, String body) {
        DefDescriptor<ComponentDef> cmpDesc = addSourceAutoCleanup(
                ...
                String.format(
                        baseComponentTag,
                        "controller='java://org.auraframework.impl.java.controller.JavaTestController' " + attrs,
    ...
    public void testGetClientRenderingAfterThemeChange() throws Exception {
        ...
        DefDescriptor<?> themeDesc = Aura.getDefinitionService().getDefDescriptor(cmpDesc,
                DefDescriptor.CSS_PREFIX, ThemeDef.class);
        addSourceAutoCleanup(themeDesc, String.format(".%s {font-style:italic;}", className));
        open(cmpDesc);
        assertEquals("italic", auraUITestingUtil.findDomElement(By.cssSelector("." + className))
                .getCssValue("font-style"));
        updateStringSource(themeDesc, String.format(".%s {font-style:normal;}", className));
        open(cmpDesc);
        assertEquals("normal", auraUITestingUtil.findDomElement(By.cssSelector("." + className))
                .getCssValue("font-style"));
    }
    ...
    public void testGetClientRenderingAfterNamespaceChange() throws Exception {
        ...
        DefDescriptor<?> themeDesc = Aura.getDefinitionService().getDefDescriptor(cmpDesc,
                DefDescriptor.CSS_PREFIX, ThemeDef.class);
        addSourceAutoCleanup(themeDesc, String.format(".%s {font-size:TOKEN;}", className));
        DefDescriptor<?> namespaceDesc = Aura.getDefinitionService().getDefDescriptor(String.format("%s://%s",
                DefDescriptor.MARKUP_PREFIX, cmpDesc.getNamespace()), NamespaceDef.class);
        addSourceAutoCleanup(namespaceDesc,
                "<aura:namespace><theme><tokens><TOKEN>8px</TOKEN></tokens></theme></aura:namespace>");
        open(cmpDesc);
        assertEquals("8px", auraUITestingUtil.findDomElement(By.cssSelector("." + className))
                .getCssValue("font-size"));
        updateStringSource(namespaceDesc,
                "<aura:namespace><theme><tokens><TOKEN>66px</TOKEN></tokens></theme></aura:namespace>");
        open(cmpDesc);
        assertEquals("66px", auraUITestingUtil.findDomElement(By.cssSelector("." + className))
                .getCssValue("font-size"));
    }
    ...
    public void testPostAfterEventChange() throws Exception {
        ...
        waitUntil(new ExpectedCondition<Boolean>() {
            @Override
            public Boolean apply(WebDriver input) {
                return "kaboom".equals(auraUITestingUtil.getEval(String.format(
                        "var e=$A.getEvt('%s');return e && e.getDef().getAttributeDefs().explode.defaultValue.value;",
                        eventDesc.getDescriptorName())));
            }
        });
    }
    ...
    public void testPostAfterLayoutChange() throws Exception {
        ...
        addSourceAutoCleanup(
                ...
                "<aura:layouts default='def'><aura:layout name='def'><aura:layoutItem container='xspot'>first</aura:layoutItem></aura:layout></aura:layouts>");
        open(appDesc);
        auraUITestingUtil.waitForElementText(By.cssSelector("#xspot"), "first", true);
        updateStringSource(
                layoutsDesc,
                "<aura:layouts default='def'><aura:layout name='def'><aura:layoutItem container='xspot'>second</aura:layoutItem></aura:layout></aura:layouts>");
        triggerServerAction();
        auraUITestingUtil.waitForElementText(By.cssSelector("#xspot"), "second", true);
    }
