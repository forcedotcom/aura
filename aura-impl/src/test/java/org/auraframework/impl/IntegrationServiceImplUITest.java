...
public class IntegrationServiceImplUITest extends WebDriverTestCase {
    ...
    /**
     * Verify using IntegrationService to inject a simple component with a Java model, Javascript Controller and Java Controller.
     */
    ...
    private DefDescriptor<ComponentDef> setupSimpleComponentWithModelAndController(){
        ...
        DefDescriptor<ControllerDef> jsControllerdesc = 
            Aura.getDefinitionService().getDefDescriptor(
                    String.format("%s://%s.%s", DefDescriptor.JAVASCRIPT_PREFIX,cmpDesc.getNamespace(), cmpDesc.getName())
                    , ControllerDef.class);
        auraTestingUtil.addSourceAutoCleanup(jsControllerdesc,
                "{" +
                    "handleClick:function(cmp){" +
                        "var a = cmp.get('c.getString');" +
                        "a.setCallback(cmp,function(a){" +
                            "cmp.find('dataFromController').getValue('v.body').push(" +
                                "$A.newCmp({componentDef: 'markup://aura:text',"+
                                    "attributes:{"+
    ...
    public void testComponentArrayAsAttribute() throws Exception{
        ...
        String attributeWithDefaultsMarkup = "<aura:attribute name='cmpsDefault' type='Aura.Component[]'>"+
                                                ...
                                                "<span class='spanDefault'>Span component as default</span>"+
                                             "</aura:attribute>{!v.cmpsDefault}";
        DefDescriptor<ComponentDef> cmpToInject = auraTestingUtil.addSourceAutoCleanup(ComponentDef.class, 
                String.format(AuraImplTestCase.baseComponentTag, "", attributeMarkup + attributeWithDefaultsMarkup));
        
        DefDescriptor<ComponentDef> customStub = addSourceAutoCleanup(ComponentDef.class, 
                getIntegrationStubMarkup("java://org.auraframework.impl.renderer.sampleJavaRenderers.RendererToInjectComponentAsAttributes",
                        false, false, false));
        ...
        assertEquals("Failed to initializing attribute of type Aura.Component[]", 
                "Div as default", devDefault.getText());
        WebElement spanDefault = findDomElement(By.cssSelector("span.spanDefault"));
        assertEquals("Failed to initializing attribute of type Aura.Component[]",
                "Span component as default", spanDefault.getText());
    ...
    public void testMissingPlaceholder() throws Exception{
        ...
        DefDescriptor<ComponentDef> customStubCmp = addSourceAutoCleanup(ComponentDef.class, 
                getIntegrationStubMarkup("java://org.auraframework.impl.renderer.sampleJavaRenderers.RendererToInjectCmpInNonExistingPlaceholder",
                        true, true, true));        
        //openIntegrationStub(customStubCmp, cmpToInject, null, "fooBared"+auraTestingUtil.getNonce());
        openIntegrationStub(customStubCmp, cmpToInject, null, "fooBared");
        //TODO RJ:Should see an error message that a placeholder was not specified. Update the test based on error message.
    }
    ...
    public void testMissingLocalId() throws Exception{
        ...
        DefDescriptor<ComponentDef> customStubCmp = addSourceAutoCleanup(ComponentDef.class, 
                getIntegrationStubMarkup("java://org.auraframework.impl.renderer.sampleJavaRenderers.RendererToInjectCmpWithNullLocalId",
                                         true, true, false));        
        Map<String, Object> attributes = Maps.newHashMap();
    ...
    public void testExceptionDuringComponentInitialization() throws Exception{
        ...
        //TODO: Update test to see that error message was displayed in error div
    }
    ...
    public void testMultipleIntegrationObjectsOnSamePage() throws Exception{
        ...
        for(String facet : facets){
            ...
            facetMarkup = facetMarkup + String.format("<%s:%s desc='%s:%s' attrMap='%s' placeholder='%s' localId='%s'/>",
                                defaultStubCmp.getNamespace(),defaultStubCmp.getName(),
                                cmpToInject.getNamespace(),cmpToInject.getName(),
        ...
        assertEquals("Framework loaded multiple times", 
                1, getDriver().findElements(By.xpath("//div//script[contains(@src,'aura_dev.js')]")).size());
    ...
    private String getIntegrationStubMarkup(String javaRenderer, Boolean attributeMap, Boolean placeHolder, Boolean localId){
        String stubMarkup = String.format("<aura:component render='server' renderer='%s'>"+
    ...
    /**
     * Pass the descriptor and attributes map of the component to be injected to a Stub component.
     * Open the stub component using webdriver.
     */
