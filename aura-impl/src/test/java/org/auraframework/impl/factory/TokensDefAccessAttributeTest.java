...
public class TokensDefAccessAttributeTest extends BaseAccessAttributeTest<TokensDef> {
    public TokensDefAccessAttributeTest() {
        super(TokensDef.class, "<aura:tokens %s>%s</aura:tokens>");
    }
    ...
    public void testDefinitionWithPrivateAccessCustomNamespace() throws Exception {
        ...
        Source<TokensDef> source = util.buildBundleSource(util.getCustomNamespace(),
                defClass, 
                Lists.newArrayList(
                    new BundleEntryInfo(type, String.format(tag, "access='PRIVATE'", ""))
                    ));
        TokensDef def = compilerService.compile(source.getDescriptor(), source);
    ...
    public void testDefinitionWithPrivateAccessInternalNamespace() throws Exception {
        AuraTestingUtil util = getAuraTestingUtil();
        Source<TokensDef> source = util.buildBundleSource(util.getInternalNamespace(),
                defClass, 
                Lists.newArrayList(
                    new BundleEntryInfo(type, String.format(tag, "access='PRIVATE'", ""))
                    ));
    ...
    public void testDefinitionWithPrivateAccessPrivilegedNamespace() throws Exception {
        AuraTestingUtil util = getAuraTestingUtil();
        Source<TokensDef> source = util.buildBundleSource(util.getPrivilegedNamespace(),
                defClass, 
                Lists.newArrayList(
                    new BundleEntryInfo(type, String.format(tag, "access='PRIVATE'", ""))
                    ));
