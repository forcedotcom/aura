...
/**
 * This class has automation to verify implementation of rendering for components.
 * There are two types of renderers: Javascript Renderers(client side) and Java Renderers(server side).
 * Javascript Renderers: Are treated as non local to the app server. They have a definition(JavascriptRendererDef)
...
public class RendererDefTest extends AuraImplTestCase {
    ...
    /**
     * Verify the structure of a ComponentDef when a component uses Java Renderer.
    ...
    public void testComponentDefWhenRedererIsJava()throws Exception{
        DefDescriptor<ComponentDef> d = Aura.getDefinitionService().getDefDescriptor("test:test_SimpleJavaRenderer", ComponentDef.class);
        ...
        assertNull("Component definitions of components which use java renderers should not have a renderer def",((Map<?,?>)defObj).get("rendererDef"));
    }
    /**
     * Verify the serialized form of Component instance when java renderers are used.
    ...
    public void testComponentInstanceDefWhenRendererIsJava()throws Exception{
        Component component = Aura.getInstanceService().getInstance("test:test_SimpleJavaRenderer", ComponentDef.class, null);
    ...
    /**
     * Verify the Creating component instance when a non existing java renderer is specified.
