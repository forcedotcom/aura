...
public abstract class BaseComponentDefTest<T extends BaseComponentDef> extends AuraImplTestCase {
    @Inject
    ...
    private void assertFalse(Object def, Method method) {
        try {
            assertEquals(String.format("%s should be false", method.getName()), Boolean.FALSE, method.invoke(def));
        } catch (Exception e) {
            throw new AuraRuntimeException(e);
        }
    }
    ...
    private void assertTrue(Object def, Method method) {
        try {
            assertEquals(String.format("%s should be true", method.getName()), Boolean.TRUE, method.invoke(def));
        } catch (Exception e) {
            throw new AuraRuntimeException(e);
        }
    }
    ...
    public void testGetRegisteredEventDefs() throws Exception {
        ...
        Assert.assertThat(registeredED, Matchers.allOf(IsMapWithSize.aMapWithSize(2),
                                                       Matchers.hasKey("parentEvent"),
                                                       Matchers.hasKey("anotherParentEvent")));
    }
    ...
    public void testHasLocalDependenciesWithServersideModel() throws Exception {
        ...
        assertEquals(Boolean.TRUE, this.serializeAndReadAttributeFromDef(baseComponentDef, Json.ApplicationKey.HASSERVERDEPENDENCIES.toString()));
    }
