...
public class ForEachTest extends AuraImplTestCase {
    ...
    /**
     * Verify iteration of a list of String, Number and Boolean values using aura:forEach
     * @throws Exception
     */
    public void testIteratingListOfBasicDataType()throws Exception{
        String namespace = "forEachDefTest";
        String cmpName = "basicDataType";
        Component cmp = (Component)Aura.getInstanceService().getInstance(String.format("%s:%s", namespace, cmpName), ComponentDef.class);
        ...
        for (Component b : innerComponentList) {
            if(b.getLocalId().equals("stringValue") ||  b.getLocalId().equals("numberValue") || b.getLocalId().equals("booleanValue")  ){
   ...
   /**
    * Verify iteration of a list of String, Number and Boolean values passed from a top level component.
    * @throws Exception
    */
    public void testIteratingListOfBasicDataType_valueByReference() throws Exception{
        ...
        //Trim the list to only consider test components
        for (Component b : innerComponentList) {
            if(b.getLocalId().equals("stringValueByRef") ||  b.getLocalId().equals("numberValueByRef") || b.getLocalId().equals("booleanValueByRef")){
                assertEquals("markup://forEachDefTest:forEachDefDisplay", b.getDescriptor().getQualifiedName());
                componentList.add(b);
            }
        }
    ...
    /**
     * Assert that value provided by top level component is the same as the Value seen by component in forEach block.
     * @throws Exception
     */
    private void assertValuesInForEachBlock(Component topCmp, String propertyOntopCmp,
                                            ArrayList<Component> componentList, int startIndex,
                                            String localId , String propertyOnlocalCmp)throws Exception{
        ...
        for(int i=0; i<3; i++){
            ...
            assertEquals("Expected to see component with ID "+localId+" but saw "+innerCmp.getLocalId(),
                    localId, innerCmp.getLocalId());
            assertEquals("Value mismatch between top component and inner component in forEach",
                    ((ArrayList<?>)values).get(i),innerCmp.getValue(new PropertyReferenceImpl(propertyOnlocalCmp,null)));
        }
    }
    /**
     * @param cmp Component which has forEach blocks, can have multiple blocks but not nested forEach blocks.
    ...
    private Collection<Component> getInnerComponents(Component cmp) throws Exception{
        ...
        Collection<Component> bodyList = (Collection<Component>)body;
        for (Component c : bodyList) {
            if(c.getDescriptor().getQualifiedName().startsWith(String.format(forEachDescriptor, cmp.getDescriptor().getNamespace(), cmp.getDescriptor().getName()))){
