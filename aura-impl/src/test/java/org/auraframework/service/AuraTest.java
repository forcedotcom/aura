 ...
 * 
 * This class generates a suite of tests based on implementations of the
 * AuraServices in the aura.service package. While those classes implement the
 * service interfaces that is only to enforce at compile-time that each method
 * on the interfaces has tests.
 * 
 * null is passed into the test implementation of the service for all arguments,
 * and the return type of the method is disregarded.
 * 
 * A property called 'service' is set on the impl before it is executed. That
 * should be used by the impl at the service instance to test.
 * 
 * Another property called 'config' is set on the impl before it is executed as
 * well. This will be one of the Config objects returned by getConfigs() on that
 * impl. Every config returned by that method will be used as a seperate test
 * suite so that the same test cases can be run multiple times with different
 * config data.
 */
public class AuraTest extends TestCase {
                ...
                if (AuraService.class.isAssignableFrom(returnType)) {
                    ...
                    if (!found) {
                        throw new AssertionFailedError(String.format(
                                "No test suite exists for %s.  Expected to find %s", returnType.getSimpleName(),
                                testSuiteName));
                    }

                    TestSuite serviceSuite = createTestSuite((Class<? extends AuraService>) returnType,
                            (Class<? extends BaseServiceTest<?, ?>>) clz, method);
    ...
    private static <T extends AuraService> TestSuite createTestSuite(Class<T> serviceClass,
        ...
        // The ftest framework indexes test cases by class name rather than
        // suite name, so if these generated
        // tests don't have unique names, some won't run in automation. We'll
        // use param types to differentiate.
        for (Method serviceMeth : serviceClass.getMethods()) {
            ...
            BaseServiceTest<T, ?> test = (BaseServiceTest<T, ?>) suiteClass.getConstructor(String.class).newInstance(
                    name);
