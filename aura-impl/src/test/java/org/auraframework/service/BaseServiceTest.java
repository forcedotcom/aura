...
/**
 */
public abstract class BaseServiceTest<T extends AuraService, C extends BaseServiceTest.Config> extends AuraImplTestCase {
    ...
    public void runTest() throws Throwable {
        // for the case generated by the dummy testNothing method, really do
        // nothing
        if (auraMethod == null) {
            return;
        }
        ...
        if (failure != null) {
            StringBuilder msg = (failure.getMessage() != null) ? new StringBuilder(failure.getMessage())
                    : new StringBuilder();
    ...
    public Object[] getParameterList(Method method) {
        ...
        for (Class<?> param : parameters) {
            // If its a primitive, null isn't sufficient, you get a invalid cast
            // exception.
            // So use a smart default.
            if (param.isPrimitive()) {
                if (param.equals(Short.TYPE)) {
                    paramsList.add(0);
                }
                if (param.equals(Integer.TYPE)) {
                    paramsList.add(0);
                }
                if (param.equals(Long.TYPE)) {
                    paramsList.add(0);
                }
                if (param.equals(Float.TYPE)) {
                    paramsList.add(0);
                }
                if (param.equals(Double.TYPE)) {
                    paramsList.add(0);
                }
                if (param.equals(Boolean.TYPE)) {
                    paramsList.add(Boolean.FALSE);
                }
            } else {
                paramsList.add(null);
            }
    ...
    /**
     * Subclasses of this class are loaded dynamically by AuraTest, so adding
     * this method so that junit doesn't get upset if there are no test methods
     * on the subclass, because in this case, that's ok.
     */
