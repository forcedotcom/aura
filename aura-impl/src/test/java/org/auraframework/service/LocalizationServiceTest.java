...
public class LocalizationServiceTest extends BaseServiceTest<LocalizationService, LocalizationServiceTest.Config> implements LocalizationService {
    ...
    public String formatDate(Date date) {
        // we will be changing the system default and want to be able to set it back to the original value
        ...
        try {
            ...
            for (Config c: getConfigs()) {
                ...
                // set the system default to be the test config Locale, since no arg is passed in to specify Locale
                Locale.setDefault(loc.getDateLocale());

                // loop through all the Dates
                for (Date d: LocalizationServiceTestData.DATES) {

                    // get the JDK standard answer (for default Locale and default Format
    ...
    public String formatTime(Date time) {
        ...
        try {
            ...
            AuraLocale loc = c.getAuraLocale();
            Locale l = loc.getDateLocale();
            // set the system default (used when no Locale passed into a format or parse method
            ...
            // make sure they match
            assertEquals("Failed for locale: " + l.getDisplayName() + " timezone: " + loc.getTimeZone().getID(), expectedDate, actual);
        } finally {
    ...
    public String formatTime(Date time, int timeStyle) {
        ...
        try {
            ...
            for (int style: LocalizationServiceTestData.DATE_FORMATS) {
                ...
                assertEquals("Failed for locale: " + l.getDisplayName() + " timezone: " + loc.getTimeZone().getID() + " style: " + style, expectedDates[i++], actual);
            }
        }
         finally {
            Locale.setDefault(originalDefault);
    ...
    public String formatTime(Calendar cal) {
        ...
        try {
            ...
            assertEquals("Failed for locale: " + l.getDisplayName() + " timezone: " + loc.getTimeZone().getID(), expectedTime, actual);
        } finally {
            Locale.setDefault(originalDefault);
            TimeZone.setDefault(originalTimeZone);
        }
    ...
    public String formatDateTime(Date dateTime) {
        ...
        try {
            ...
            // make sure they match
            assertEquals("Failed for locale: " + l.getDisplayName() + " timezone: " + loc.getTimeZone().getID(), expectedDateTime, actual);
        } finally {
            // set the default Locale back
            Locale.setDefault(originalDefault);
            TimeZone.setDefault(originalTimeZone);
    ...
    public String formatDateTime(Date dateTime, int dateStyle, int timeStyle) {
        ...
        try {
            ...
            assertEquals("Failed for locale: " + l.getDisplayName() + " timezone: " + loc.getTimeZone().getID() + " dateStyle: 0 timeStyle: 0", expectedDateTime, actual);
        } finally {
            Locale.setDefault(originalDefault);
            TimeZone.setDefault(originalTimeZone);
        }
    ...
    public String formatDateTime(Date dateTime, Locale locale, TimeZone timeZone) {
        //refer to DateServiceTest.testGetDateTimeStyleConverter_locale_dateStyle_timeStyle
        return null;
    }
    ...
    public String formatDateTime(Calendar cal, int dateStyle, int timeStyle) {
        ...
        try {
            ...
            assertEquals("Failed for locale: " + l.getDisplayName() + " timezone: " + loc.getTimeZone().getID() + " dateSstyle: 0 timeStyle: 0", inputTestDateTimeStringStyle00, actual);
        } finally {
            Locale.setDefault(originalDefault);
        }
        return null;
    ...
    public String formatCurrency(BigDecimal value, Locale locale) {
        for (Config c: getConfigs()) {
            ...
            Locale l = loc.getNumberLocale();
            for (BigDecimal n : LocalizationServiceTestData.CURRENCY_BIGDECIMALS) {
                DecimalFormat df = (DecimalFormat)DecimalFormat.getCurrencyInstance(l);
                df.setParseBigDecimal(true);
                String expected = df.format(n);
                String actual = service.formatCurrency(n,l);
                assertEquals(expected, actual);
        ...
        try {
            ...
            for (Config c: getConfigs()) {
                ...
                for (Date d: LocalizationServiceTestData.DATES) {
                    ...
                    // get the JDK standard starting value (the date String for the default Locale)
    ...
    public Date parseTime(String time) throws ParseException {
        ...
        try {
            ...
            assertEquals("Failed for locale: " + l.getDisplayName() + " timezone: " + loc.getTimeZone().getID(), expectedTime, actual.toString());
        } finally {
            // always reset the default Locale
            Locale.setDefault(originalDefault);
            TimeZone.setDefault(originalTimeZone);
    ...
    public Date parseTime(String time, int timeStyle) throws ParseException {
        ...
        try {
            ...
            assertEquals("Failed for locale: " + l.getDisplayName() + " timezone: " + loc.getTimeZone().getID() + " style: 2", expectedTime, actual.toString());
    ...
    public Date parseDateTime(String dateTime, int dateStyle, int timeStyle) throws ParseException {
        ...
        try {
           ...
           assertEquals("Failed for locale: " + l.getDisplayName() + " timezone: " + loc.getTimeZone().getID(), expectedDateTime, actual.toString());
        } finally {
            Locale.setDefault(originalDefault);
        }
        return null;
        ...
        try {
            for (Config c: getConfigs()) {
                ...
                for (int n: LocalizationServiceTestData.INTS) {
                    ...
                    // get the JDK standard starting value (the localized number String for the default Locale)
        ...
        for (Config c: getConfigs()) {
            ...
            for (String n: LocalizationServiceTestData.FAIL_INT_STRINGS) {
                try{
                    // parse it through Aura Localization Service
                    service.parseInt(n, l);
                    assertTrue("# Exception not thrown for invalid {int} test data:"+n+" & locale: "+l.getDisplayName(), false);
                }
                catch(Exception e){
        ...
        for (Config c: getConfigs()) {
            ...
            for (String n: LocalizationServiceTestData.FAIL_LONG_STRINGS) {
                try{
                    // parse it through Aura Localization Service
                    service.parseLong(n, l);
                    assertTrue("# Exception not thrown for invalid {long} test data:"+n+" & locale: "+l.getDisplayName(), false);
                }
                catch(Exception e){
                    assertTrue("# Incorrect exception type!", ((e instanceof ParseException)));
                }
        ...
        for (Config c: getConfigs()) {
            ...
            for (String n: LocalizationServiceTestData.PASS_FLOAT_STRINGS) {
                try{
                    ...
                    assertTrue("# Exception thrown for valid {float} test data:"+n+" & locale:"+l.getDisplayName()+"\n"+ e.getMessage(), false);
                }
            }
            ...
            for (String n: LocalizationServiceTestData.FAIL_FLOAT_STRINGS) {
                try{
                    // parse it through Aura Localization Service
                    service.parseFloat(n, l);
                    assertTrue("# Exception not thrown for invalid {float} test data:"+n+" & locale: "+l.getDisplayName(), false);
                }
                catch(Exception e){
                    assertTrue("# Incorrect exception type!", ((e instanceof ParseException)));
                }
        ...
        for (Config c: getConfigs()) {
            ...
            for (String n: LocalizationServiceTestData.PASS_DOUBLE_STRINGS) {
                try{
                    ...
                    assertTrue("# Exception thrown for valid {double} test data:"+n+" & locale:"+l.getDisplayName()+"\n"+ e.getMessage(), false);
                }
            }
            ...
            for (String n: LocalizationServiceTestData.FAIL_DOUBLE_STRINGS) {
                try{
                    // parse it through Aura Localization Service
                    service.parseDouble(n, l);
                    assertTrue("# Exception not thrown for invalid {double} test data:"+n+" & locale: "+l.getDisplayName(), false);
                }
                catch(Exception e){
                    assertTrue("# Incorrect exception type!", ((e instanceof ParseException)));
                }
        ...
        for (Config c: getConfigs()) {
            ...
            for (String n: LocalizationServiceTestData.PASS_PERCENT_STRINGS) {
                try{
                    ...
                    assertTrue("# Exception thrown for valid percent test data:"+n+" & locale:"+l.getDisplayName()+"\n"+ e.getMessage(), false);
                }
            }
            // loop through all the hard coded test data expected to fail
            for (String n: LocalizationServiceTestData.FAIL_PERCENT_STRINGS) {
                try{
                    // parse it through Aura Localization Service
                    service.parsePercent(n, l);
                    assertTrue("# Exception not thrown for invalid percent test data:"+n+" & locale: "+l.getDisplayName(), false);
                }
                catch(Exception e){
                    assertTrue("# Incorrect exception type!", ((e instanceof ParseException)));
                }
    ...
    public String formatNumber(BigDecimal number, Locale locale) {
        for (Config c: getConfigs()) {
            AuraLocale loc = c.getAuraLocale();
            Locale l = loc.getNumberLocale();
            DecimalFormat df = (DecimalFormat)DecimalFormat.getNumberInstance(l);
            df.setParseBigDecimal(true);
            for (BigDecimal n : LocalizationServiceTestData.CURRENCY_BIGDECIMALS) {
                String expected = df.format(n);
                String actual = service.formatNumber(n,l);
