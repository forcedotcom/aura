...
public class MockingUtilTest extends WebDriverTestCase {
    ...
    public void testMockModel() throws Exception {
        ...
        DefDescriptor<ApplicationDef> appDescriptor = auraTestingUtil.addSourceAutoCleanup(ApplicationDef.class, String
                .format(baseApplicationTag, String.format("model='%s'", modelDefDescriptor.getQualifiedName()),
        ...
        // mock list property too
        Aura.get(TestContextAdapter.class).getTestContext().getLocalDefs().clear();
        modelProperties = ImmutableMap
                .of("string", (Object)"override", "stringList", Lists.newArrayList("X", "Y", "Z"));
        mockingUtil.mockModel(modelDefDescriptor, modelProperties);
        open(appDescriptor);
        assertEquals("overrideXYZ", getText(By.cssSelector("body")));

        // If we want to "chain" mocks (where it may be instantiated multiple times in a single request), Mockito lets
        // us do it, but we don't have a convenience function for that because it shouldn't be a common case (in tests)
        Aura.get(TestContextAdapter.class).getTestContext().getLocalDefs().clear();
        ...
        // rather than build another component, we'll just instantiate the same component consecutively
        open(appDescriptor);
    ...
    public void testMockServerProvider() throws Exception {
        ...
        // no mocking - provider output isn't valid for this component (or any probably), but the def is valid
        openNoAura(url);
        assertTrue(getText(By.cssSelector("body")).contains(
                String.format("%s did not provide a valid component", providerDefDescriptor.getQualifiedName())));
