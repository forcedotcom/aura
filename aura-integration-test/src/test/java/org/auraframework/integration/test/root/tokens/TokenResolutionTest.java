...
public class TokenResolutionTest extends StyleTestCase {
    ...
    public void testAppTokensWhenExplicitOnly() throws Exception {
    ...
    /** test that the correct app tokens are used under various combinations of which files exist */
    public void testAppTokensWhenImplicitOnly() throws Exception {
        // component in a different ns, with ns default tokens
        addNsTokensOtherNamespace(tokens().token("color", "red"));
        DefDescriptor<StyleDef> toTest = addStyleDefOtherNamespace(".THIS {color: token(color)}");
        ...
        // the app's namespace-default tokens should override the components namespace-default tokens
        // because the app implicitly uses its ns defaults as overrides
        assertStyle(toTest, ".THIS {color:blue}");
    }

    /** test that the correct app tokens are used under various combinations of which files exist */
    public void testAppTokensWhenEmpty() throws Exception {
        // component in a different ns, with ns default tokens
        addNsTokensOtherNamespace(tokens().token("color", "red"));
        DefDescriptor<StyleDef> toTest = addStyleDefOtherNamespace(".THIS {color: token(color)}");

        // our app is in a different ns from the cmp, specifying an empty string for the tokens
        addContextApp(String.format("<aura:application tokens=''/>"));

        // no override should be set, so it should fallback to the cmp's namespace-defaults
        assertStyle(toTest, ".THIS {color:red}");
    }

    /** test that the correct app tokens are used under various combinations of which files exist */
    public void testAppTokensWhenExplicitAndImplicit() throws Exception {
        // component in a different ns, with ns default tokens
        addNsTokensOtherNamespace(tokens().token("color", "red"));
        DefDescriptor<StyleDef> toTest = addStyleDefOtherNamespace(".THIS {color: token(color)}");
    ...
    /** test that the correct app tokens are used under various combinations of which files exist */
    public void testAppTokensDoNotContainToken() throws Exception {
        // component in a different ns, with ns default tokens
        addNsTokensOtherNamespace(tokens().token("color", "red"));
        DefDescriptor<StyleDef> toTest = addStyleDefOtherNamespace(".THIS {color: token(color)}");
    ...
    /** test that inherited token is applied */
    public void testExplicitAppTokensInheritedToken() throws Exception {
        // component in a different ns, with ns default tokens
        addNsTokensOtherNamespace(tokens().token("color", "red"));
        DefDescriptor<StyleDef> toTest = addStyleDefOtherNamespace(".THIS {color: token(color)}");
    ...
    /** test that an *implicit* app tokens inherited token is applied */
    public void testImplicitAppTokensInheritedToken() throws Exception {
        // component in a different ns, with ns default tokens
        addNsTokensOtherNamespace(tokens().token("color", "red"));
        DefDescriptor<StyleDef> toTest = addStyleDefOtherNamespace(".THIS {color: token(color)}");

        // the namespace-default for the app inherits the tokens
        DefDescriptor<TokensDef> parent = addSeparateTokens(tokens().token("color", "blue"));
        addNsTokens(tokens().parent(parent));
        ...
        // the inherited value from the implicit tokens should be used
        assertStyle(toTest, ".THIS {color:blue}");
    }

    /** test when implicit app tokens inherit from the other cmp's namespace-defaults */
    public void testImplicitAppTokensCircleAround() throws Exception {
        // component in a different ns, with ns default tokens
        DefDescriptor<TokensDef> other = addNsTokensOtherNamespace(tokens().token("color", "red"));
        DefDescriptor<StyleDef> toTest = addStyleDefOtherNamespace(".THIS {color: token(color)}");

        // the app's implicit tokens inherits from the cmp's namespace-default tokens
        addNsTokens(tokens().parent(other));
    ...
    /** test when implicit app tokens inherit from the other cmp's namespace-defaults, but then changes the val */
    public void testImplicitAppTokensCircleAroundChanged() throws Exception {
        // component in a different ns, with ns default tokens
        DefDescriptor<TokensDef> other = addNsTokensOtherNamespace(tokens().token("color", "red"));
        DefDescriptor<StyleDef> toTest = addStyleDefOtherNamespace(".THIS {color: token(color)}");

        // the app's implicit tokens inherits from the cmp's namespace-defaults
        addNsTokens(tokens().parent(other).token("color", "blue"));
