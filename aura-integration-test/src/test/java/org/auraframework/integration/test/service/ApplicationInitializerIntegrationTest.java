...
public class ApplicationInitializerIntegrationTest extends IntegrationTestCase {
    ...
    @Test
    public void testValidInitializerNames() throws Exception {
        ...
        for (ApplicationInitializer initializer : initializerMap.values()) {
            ...
            Assert.assertThat("Initializer "+initializer.getClass().getName()+" needs a valid name",
                    annotation.name(), Matchers.matchesPattern("^[A-Za-z_][A-Za-z_0-9]*$"));
        }
    }

    @Test
    public void testValidInitializerDuplicates() throws Exception {
        Map<String,ApplicationInitializer> initializerMap;
        initializerMap = applicationContext.getBeansOfType(ApplicationInitializer.class);;
        ...
        for (ApplicationInitializer initializer : initializerMap.values()) {
            ...
            String value = initializer.getClass().getName();
            for (String appName : annotation.applications()) {
                String key = annotation.name() + " -> " + appName;
                Assert.assertThat("Found duplicate map entries for "+key
                        +" found "+value+", and "+duplicateCheckMap.get(key),
                        duplicateCheckMap.get(key), Matchers.nullValue());
                duplicateCheckMap.put(key, value);
            }
        }
    }
