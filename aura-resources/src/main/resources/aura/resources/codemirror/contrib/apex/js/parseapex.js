/**
 * Apex parser for codemirror
 *
 *
 */

var ApexParser = Editor.Parser = (function() {
  ...
  function ApexLexical(indented, column, type, align, prev, info) {
  ...
  // apex indentation rules.
  function indentApex(lexical) {
  ...
  function parseApex(input, basecolumn) {
    // Wrap the input in a token stream
    var tokens = tokenizeApex(input);
    ...
    var lexical = new ApexLexical(basecolumn || 0, 0, "block", false);
    ...
    function next(){
      ...
      if (token.content == "\n"){
        ...
        // them.
        token.indentation = indentApex(lexical);

      }
      // No more processing for meaningless tokens.
      if (token.type == "whitespace" || token.type == "comment" || token.type == "apexdoc" || token.type == "annotation")
        return token;
    ...
    function copy(){
      ...
      return function copyParser(input){
        ...
        column = indented = 0;
        tokens = tokenizeApex(input, _tokenState);
        return parser;
      };
    }
    ...
    function pushlex(type, info) {
      var result = function(){
        lexical = new ApexLexical(indented, column, type, null, lexical, info)
      };
      result.lex = true;
      return result;
    }
