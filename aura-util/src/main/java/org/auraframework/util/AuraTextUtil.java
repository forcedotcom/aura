...
public class AuraTextUtil {
    ...
    public static String urldecode(String url) {
        try {
            ...
            // Something went really wrong if UTF-8 isn't supported and we get
            // here
    ...
    public static String replaceChar(String value, char ch, CharSequence replacement) {
        ...
        StringBuilder buf = new StringBuilder(value.length() + 16); // some
                                                                    // extra
                                                                    // space
        int j = 0;
        while (i != -1) {
    ...
    public static String replaceSimple(String s, String src, String target) {
        ...
        if (target == null) {
            target = "null"; // gag - but this is the way the replaceRegex
                             // works, so, I guess we need to be compatible
        }
        ...
        buf.append(s.substring(0, pos)).append(target); // replace the first
                                                        // instance
        pos += src.length();
