...
/**
 * A minimal {@link java.io.InputStreamReader} implementation that reads only what it needs from the wrapped
 * {@link InputStream} and not a byte more. Java's built-in {@code InputStreamReader} can read ahead, but
 * that doesn't play well with use cases that mix binary and text data in one stream, so this
 * {@code InputStreamReader} variant was created.<br>
 * <br>
 * Note that its original user, {@link org.auraframework.util.json.JsonStreamReader}, reads only one
 * character at a time, so that is how this implementation is optimized. This can be updated if other use
 * cases come along. At the very least, this {@code InputStreamReader} does intelligently handle the first
 * byte in a UTF-8 character by bulk-reading the remaining bytes based on the character length, which is
 * encoded into the first byte in a UTF-8 character. Another optimization is having one-byte UTF-8 characters
 * bypass the {@link CharsetDecoder}.<br>
 * <br>
 * This {@link Reader} assumes that characters in the {@code InputStream} are all UTF-8. Byte order mark
 * (BOM) characters encoded as UTF-8 are dropped, since their presence is nonsensical in UTF-8 (and we don't
 * want to potentially contaminate any UTF-16 outputs). 5-byte and 6-byte UTF-8 byte sequences are dropped at
 * this time. Bytes with values {@code 0xfe} or {@code 0xff} are dropped as neither are valid UTF-8
 * bytes.<br>
 * <br>
