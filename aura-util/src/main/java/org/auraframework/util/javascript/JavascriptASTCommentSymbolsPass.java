...
public class JavascriptASTCommentSymbolsPass implements CompilerPass {
    ...
    private class FindRegExp extends AbstractPostOrderCallback {
        private final String ESCAPED_TARGET = "\\*";
        private final String ESCAPED_REPLACE = "\\u002A";
        private final String REGEX_TARGET = "*";
        private final String REGEX_REPLACE = "{0,}";

        private final String CLOSE_COMMENT = "*/";
        private final String CLOSE_COMMENT_PATTERN = "#_#";
        ...
        private String replaceWildCard(String str) {
            // converts escaped char first to avoid mis-replacing
            return str.replace(ESCAPED_TARGET, ESCAPED_REPLACE).replace(REGEX_TARGET, REGEX_REPLACE);
        }
        ...
        public void visit(NodeTraversal t, Node n, Node parent) {
            ...
            if (n.isRegExp()) {
                ...
                String regex = replaceWildCard(regexStringNode.getString());
                regexStringNode.setString(regex);
            }

            // Replace the * in last string node in RegExp constructor
            else if(Node.SYNTHETIC_BLOCK_PROP == n.getType() && "RegExp".equals(n.getString())) {
                // starts from the content of RegExp
                Node cursor = n.getNext();
                while(cursor != null) {
                    if(cursor.isString() && cursor.getString().contains(REGEX_TARGET)) {
                        String regex = replaceWildCard(cursor.getString());
                        cursor.setString(regex);
                    }
                    // RegExp object is converted into regex literal (/ /) in some cases in minified code.
                    // The only mattered case is the * as the last character.
                    cursor = cursor.getLastChild();
                }
            }

            // If we found */ within a string, we need to do some crazy refactor s
            else if (n.isString() && n.getString().contains(CLOSE_COMMENT)) {
                ...
                // String: "*/" is transformed into: "*#_#/".replace(/\*#_#/g, "*")
