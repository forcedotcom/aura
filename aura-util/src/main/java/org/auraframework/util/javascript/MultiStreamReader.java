...
public class MultiStreamReader extends Reader {
	...
	public MultiStreamReader(URL[] urls) {
		current = null;
		index = -1;
		this.urls = urls == null ? new URL[0] : Arrays
				.copyOf(urls, urls.length);
	}

	public MultiStreamReader(Collection<URL> urls) {
		current = null;
		index = -1;
		this.urls = urls == null ? new URL[0] : urls.toArray(new URL[urls
				.size()]);
	}
	...
	public int read(char[] buffer, int start, int maxRead) throws IOException {
		...
		while (maxRead > 0) {
			...
			if (thisRead == 0) {
				// Full, or stalling: done for now
				return readSoFar;
			} else if (thisRead < 0) {
				// done with this stream, check next stream on next iteration
				current.close();
				current = null;
			} else {
