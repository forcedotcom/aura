 ...
 *
 * May or may not follow the official JSON (JavaScript Object Notation) standard. It handles
 * serializing all the basics, numbers, strings, booleans, arrays, and maps as well as some common SFDC data structures
 * like PicklistItem and anything that implements {@link JsonSerializable}. Some significant consumers of the output are
 * inline editing
 * <ol>
 ...
 * </ol>
 *
 * NOTE: the code for handling the stacks is rather more complicated to maintain performance. The problem is that
 * {@link #writeMapBegin()} and {@link #writeArrayBegin()} are called hundreds of thousands of times a second,
 * meaning that creating and discarding objects for each one is too expensive. This means that we use a separate
 * stack in the case that we are not formatting to avoid the allocation of the object.
 *
 * @see <a href="https://sites.google.com/a/salesforce.com/user-interface/documentation/json">SFDC json
 *      documentation</a>
 * @since 144
 */
public class Json {
    ...
    /**
     * Create a Json Serialization context object that maintains information about one run. This Object is NOT
     * thread-safe. It should only be used by one thread at a time, and should not be reused.
     *
     * @param out
     *            The Appendable to write the serialized objects to.
     * @param format
     *            defaults to false. If true, the output will be multi-line and indented.
     * @param refSupport
     *            If true, any objects annotated with &#64;Serialization will be serialized using serRefIds
     */
    ...
    // This annotations is only looked for on classes that implement JsonSerializable
    ...
    public @interface Serialization {
        ...
        public enum ReferenceType {
            ...
            /**
             * If a.equals(b), just output serRefId=<the refId of the object> after the first time it's output
             */
            EQUALITY,

            /**
             * If a == b, just output serRefId=<the refId of the object> after the first time it's output
             */
            IDENTITY
        }
    ...
    /**
     * Following are a bunch of static serialize methods. They mainly exist in order to size a StringBuilder for you to
     ...
     * @param out
     *            The destination for the serialized form
     * @param format
     *            true if output should be indented and multiline for human readability (default = false)
     * @param refSupport
     *            true if @Serialization annotations should be honored (default = false)
     * @throws JsonSerializationException
     *             if there's an issue during serialization
     */
    ...
    /**
     * Creates a Json instance that is suitable for output streaming, one element at a time. This can help avoid
     * building up an entire JavaScript AST all in memory before it gets serialized, which can help cut down memory use.<br>
     * <br>
     * Note that you will need to call {@link #close()} when you are done to ensure that all characters have been
     * written out to the given OutputStream. Otherwise, some characters might be missing at the end.
     *
     * @param out
     *            The OutputStream to write the serialized objects to using UTF-8. This must not be null.
     * @param format
     *            Defaults to false. If true, the output will be multi-line and indented.
     * @param refSupport
     *            If true, any objects annotated with &#64;Serialization will be serialized using serRefIds
     * @param nullValues
     *            When true, null values are written out when they exist in arrays and map values. When false, array
     *            items and map entries with null values are not serialized
     * @return A new Json instance that you can use for streaming to the given OutputStream
     */
     ...
     *            The OutputStream to write the serialized objects to using UTF-8. This must not be null.
     * @param context
     *            The JSON serialization context to use for output
     * @return A new Json instance that you can use for streaming to the given OutputStream
     */
    ...
    /**
     * This method is essentially here to provide type-checking for the outermost map.
     *
     * @param jsonMap
     * @param out
     * @throws JsonSerializationException
     ...
     *
     * This either creates a new IndentEntry and pushes a value on the boolean stack, or it just
     * uses the boolean stack (in the case of not pretty printing).
     *
     ...
     *
     * This both checks the type on the stack, and pulls it off.
     * See the notes on performance on the class above.
     */
     ...
     * Start a comment.
     *
     * This is probably not needed, but if we do want to write a multiline comment in parts,
     * you would call this function followed by multiple calls to {@link #writeCommentBody(String)}
     * followed by a call to {@link #writeCommentEnd()}
     */
     ...
     * Write out a comment.
     *
     * Note that these are not legal structures in JSON, perhaps we should have a flag to turn off the
     * writing of these for 'valid' JSON. Unfortunately, we'd also have to rework our error handling.
     *
     * This could take the body and re-work newlines with the indent, but that seems a good
     * bit of work for little gain (i.e. the result would be prettier, but who cares).
     *
    ...
    /**
     * Write the beginning of an array. Make sure to call writeArrayEnd later on.
     *
     * @throws IOException
     */
    ...
    /**
     * If any entries have already been written to the current map/array (as marked by the write*Begin methods), write a
     * comma. If no elements have yet been written, no-op.
     *
     * @throws IOException
     */
    ...
    /**
     * Encode the given value and if m != null then perform Aura-specific seialization that outputs extra information
     * in the stream so that references can be established by the JSON reader
     */
    public void writeValue(Object value) throws IOException {
        JsonSerializer<Object> serializer = serializationContext.getSerializer(value);
        if (serializer == null) { throw new JsonSerializerNotFoundException(value); }
    ...
    /**
     * Write the date in the ISO-8601 format that's semi-standard in json2 (in that it's in the comments)
     *
     * @param value
     * @throws IOException
     */
    ...
    /**
     * Write a value into the current array, and add leading commas and formatting as appropriate.
     *
     * @param value
     * @throws IOException
     */
    ...
    /**
     * Write a value into the current Map, and add leading commas and formatting as appropriate.
     *
     * @param key
     * @param value
     * @throws IOException
    ...
    /**
     * Write a partial Map Entry -- everything except the value. This is useful when the value requires special
     * serialization.
     *
     * @param key
     * @throws IOException
    ...
    /**
     * Start a binary stream using the given length and return an OutputStream that the caller can write its binary data
     * to.<br>
     * <br>
     * After calling this, write exactly the number of bytes specified to the OutputStream returned by this method.
     * After you do that, call {@link #writeBinaryStreamEnd()}.
     *
     * @param streamLength
     *            The number of bytes that will exist in the output before the ending backtick
    ...
    public OutputStream writeBinaryStreamBegin(long streamLength) throws IOException {
        ...
        if (currentBinaryStream != null) { throw new IllegalStateException("Previous binary stream was not ended"); }
        ...
        // Wrap our binaryOutput in a CountingOutputStream so that we can validate the length later
    ...
    private void validateBinaryStreamEnabledAndWriteBacktick() throws IOException {
        if (binaryOutput == null) { throw new IllegalStateException(
                "Binary streams are supported only when Json.createJsonStream is used with an InputStream"); }
        out.append('`');
    }

    /**
     * Ends the current binary stream and ensures that the correct number of bytes were written. If a discrepancy
     * exists, then an IllegalStateException gets thrown.
     */
    public void writeBinaryStreamEnd() throws IOException {

        // Ensure that we are in a binary stream, and validate the length
        if (currentBinaryStream == null) { throw new IllegalStateException("Binary stream was not started"); }
        if (currentBinaryStreamLength != currentBinaryStream.getCount()) { throw new IllegalStateException(
                "Length of the binary stream was written out as " + currentBinaryStreamLength + " bytes, but "
                        + currentBinaryStream.getCount()
    ...
    /**
     * Writes out any buffered characters in the OutputStreamWriter to the binary OutputStream and then closes the
     * OutputStream.<br>
     * <br>
     * Note that this method does nothing if Json was not created with an OutputStream, such as via
    ...
    /**
     * Note: You should always try to use the write* methods instead, if at all possible.
     *
     * @return the appendable for this run in case you want to write something special to it.
     */
    ...
    /**
     * Resolve references and remove refId/serRefIds from the passed in object. Useful when parsing json serialized with
     * reference support by this class.
     *
     * @param config
     *            Must be a Map or List that consists only of other Maps/Lists and primitives
     * @return A Map or List representing the data passed in with its references resolved
     */
