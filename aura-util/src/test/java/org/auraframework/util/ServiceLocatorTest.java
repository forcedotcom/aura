...
public class ServiceLocatorTest extends UnitTestCase {
    ...
    /**
     * Test that ServiceLocator can provide an implementation of Specified
     * Interface. If there are multiple implementations, then the implementation
    ...
    public void testGet() {
        ...
        // 2 An interface with multiple implementation and one of them marked as
        // primary
        verifyServiceLocated(MultipleWith1PrimaryService.class, MultipleWith1PrimaryServiceImpl2.class);

        // 3 An interface with multiple implementation and none marked as
        // primary
        try {
        ...
        // 9 A concrete Service class exposing itself and also has children who
        // are exposed to ServiceLocator.
        verifyServiceLocated(SelfImplementingService.class, SelfImplementingService.class);

        // 10 Inheritance and service location
        // a. A parent Interface has a child interface, A class implementing the
        // child interface, ServiceLocator trying to find implementations of
        // Parent Interface
        // b. A parent Interface has a child interface and a class implementing
        // it, the child interface also has a class implementing it. What does
        // the ServiceLocator
        // fetch when asked for Implementation of parent interface and child
        // interface
        try {
        ...
        // 11 Other boundary cases, like non static method, without @Impl
        // annotation, public access qualifier, non-empty argument in
        // configuration
    ...
    public void testGetAll() {
        ...
        // 2 An interface with multiple implementation and one of them marked as
        // primary
        verifyAllServicesLocated(MultipleWith1PrimaryService.class, new Class<?>[] {
                MultipleWith1PrimaryServiceImpl1.class, MultipleWith1PrimaryServiceImpl2.class });

        // 3 An interface with multiple implementation and none marked as
        // primary
        verifyAllServicesLocated(MultipleImplNoPrimaryService.class, new Class<?>[] {
                MultipleImplNoPrimaryServiceImpl1.class, MultipleImplNoPrimaryServiceImpl2.class });

        // 4 An interface with multiple implementation and more than one primary
        verifyAllServicesLocated(MultipleWith2PrimaryService.class, new Class<?>[] {
                MultipleWith2PrimaryServiceImpl1.class, MultipleWith2PrimaryServiceImpl2.class });
        ...
        // 9 A concrete Service class exposing itself and also has children who
        // are exposed to ServiceLocator.
        verifyAllServicesLocated(SelfImplementingService.class, new Class<?>[] { SelfImplementingService.class,
                SelfImplementingChildService.class });
        ...
        // fetch when asked for Implementation of parent interface and child
        // interface
        verifyAllServicesLocated(InheritanceParentService.class, new Class<?>[] { InheritanceChildImpl.class,
                InheritanceParentImpl.class });
        ...
        verifyAllServicesLocated(NamedService.class,
                new Class<?>[] { NamedServiceImpl1.class, NamedServiceImpl2.class });
    }
    ...
    public void testGetWithName() {
        ...
        // There is no service registered to implement SimpleService with name
        // "IGOTTANAME"
    ...
    private <X, Y> void verifyServiceLocated(Class<X> serviceInterface, Class<Y> serviceProvider) {
        ...
        assertTrue(
                "Expected: " + serviceProvider.getSimpleName() + " for service interface: "
                        + serviceInterface.getSimpleName() + ", but got: " + sis.getClass().getSimpleName() + ".",
    ...
    private <X> void verifyAllServicesLocated(Class<X> serviceInterface, Class<?>[] serviceProviders) {
        ...
        assertEquals(
                "Number of service implementation expected(" + serviceProviders.length + ") and actual(" + sis.size()
                        + ") differ.", serviceProviders.length, sis.size());
        for (int i = 0; i < serviceProviders.length; i++) {
            Iterator<X> it = sis.iterator();
            boolean flag = false;
            while (it.hasNext()) {
                X item = it.next();
                if (serviceProviders[i].isInstance(item)
                        && serviceProviders[i].getSimpleName().equals(item.getClass().getSimpleName())) {
                    flag = true;
                    break;
                }
            }
            assertTrue("Failed to locate the provider " + serviceProviders[i].getSimpleName() + " for this service:"
                    + serviceInterface.getSimpleName(), flag);
        }
    }
