...
public class DateServiceTest extends UnitTestCase {
     ...
     * Test Data
     */
    final Date[] DATE_TIME = {
            new DebugDate(1), // now
            new DebugDate(1000L), // 12:00:01 AM GMT
            new DebugDate(1333322872649L), // 4:27:52.649 PM PDT (GMT-7)
            new DebugDate(0) // 00:00:00.000 GMT
    };
    public static final int[] DATE_TIME_STYLES = {DateFormat.SHORT, DateFormat.MEDIUM, DateFormat.LONG, DateFormat.FULL, -1};
    public static final String[] SIMPLE_DATE_FORMAT_PATTERNS = {"yyyy.MM.dd G 'at' HH:mm:ss z",
                                                                "EEE, MMM d, ''yy",
                                                                "h:mm a",
                                                                "hh 'o''clock' a, zzzz",
                                                                "K:mm a, z",
    ...
    /** NOTE: api's converter.format(date) and converter.parse(date)
     *  will rely on the TimeZone.getDefault() value to do their
     *  job. Thus, tests for these will return different results
     *  based on default TimeZone that is set.
     */
    ...
    public void testGetDateTimeISO8601Converter() throws Exception {
        ...
        for (LocaleConfig c: getConfigs()) {
            ...
            for (Date d: DATE_TIME) {
                ...
                String text = "Input date:" + d.toString() + "\t\tFormatted date:" + formattedDate + "\t\tParsed date:" + parsedDate.getTime() + "\t\tTimezone:" + tz.getID() + "\n";
                sb.append(text);
            }
        }
    ...
    public void testGetGenericISO8601Converter() throws Exception {
        ...
        for (LocaleConfig c: getConfigs()) {
            ...
            for (Date d: DATE_TIME) {
                ...
                //parsing
                Date parsedDate = converter.parse(formattedDate, tz);
                text = "Input date:" + d.toString() + "\t\tFormatted date:" + formattedDate + "\t\tParsed date:" + parsedDate.getTime() + "\t\tTimezone:" + tz.getID() + "\n";
                sb.append(text);
            }
        }
        ...
        for (LocaleConfig c: getConfigs()) {
            ...
            for(String d : DATETIME_NOSECONDS){
                try{
                    ...
                    text = "Input date:" + d.toString() + "\t\tFormatted date:" + formattedDate + "\t\tParsed date:" + parsedDate.getTime() + "\n";
                    sb.append(text);
                }
            ...
            for (Date d: DATE_TIME) {
                for(int ds : DATE_TIME_STYLES){
                    if(ds > -1){
                        ...
                        try{
                            Date parsedDate = converter.parse(formattedDate, tz);
                            text = "Input date:" + d.toString() + "\t\tFormatted date:" + formattedDate + "\t\tParsed date:" + parsedDate.getTime() + "\t\tLocale:" + l.getDisplayName() + "\t\tTimeZone: " + tz.getID() + "\t\tDate style: " + ds + "\n";
                            sb.append(text);
                        }
                        catch(IllegalArgumentException e){
                            sb.append(e.getMessage()+"\n");
            ...
            for (Date d: DATE_TIME) {
                for(int ts : DATE_TIME_STYLES){
                    if(ts > -1){
                        ...
                        try{
                            Date parsedDate = converter.parse(formattedDate, tz);
                            text = "Input date:" + d.toString() + "\t\tFormatted date:" + formattedDate + "\t\tParsed date:" + parsedDate.getTime() + "\t\tLocale:" + l.getDisplayName() + "\t\tTimeZone: " + tz.getID() + "\t\tTime style: " + ts + "\n";
                            sb.append(text);
                        }
                        catch(IllegalArgumentException e){
                            sb.append(e.getMessage()+"\n");
                ...
                for(int ds : DATE_TIME_STYLES){
                    for(int ts : DATE_TIME_STYLES){
                        if((ds+ts) > 0){
                            ...
                            try{
                                Date parsedDate = converter.parse(formattedDate, tz);
                                text = "Input date:" + d.toString() + "\t\tFormatted date:" + formattedDate + "\t\tParsed date:" + parsedDate.getTime() + "\t\tLocale:" + l.getDisplayName() + "\t\tTimeZone: " + tz.getID() + "\t\tDate style: " + ds + "\t\tTime style: " + ts + "\n";
                                sb.append(text);
                            }
                            catch(IllegalArgumentException e){
                                sb.append(e.getMessage()+"\n");
        ...
        for (LocaleConfig c: getConfigs()) {
            ...
            for (Date d: DATE_TIME) {
                for(String pattern : SIMPLE_DATE_FORMAT_PATTERNS) {
                    ...
                    try{
                        Date parsedDate = converter.parse(formattedDate, tz);
                        text = "Input date:" + d.toString() + "\t\tFormatted date:" + formattedDate + "\t\tParsed date:" + parsedDate.getTime() + "\t\tLocale:" + l.getDisplayName() + "\t\tTimeZone:" + tz.getID() + "\t\tSimpleDateFormat pattern:" + pattern + "\n";
                        sb.append(text);
                    }
                    catch(IllegalArgumentException e){
                        sb.append(e.getMessage()+"\n");
    ...
    public void testDateTimeNoneConverter(){
        try{
            ...
            assertEquals("# Incorrect exception message for api getDateTimeStyleConverter(Locale.US, -1, -1)", "Style '--' is invalid", e.getMessage());
        }
        ...
        try{
            ...
            assertEquals("# Incorrect exception message for api getDateStyleConverter(Locale.US, -1)", "Style '--' is invalid", e.getMessage());
        }
        ...
        try{
            ...
            assertEquals("# Incorrect exception message for api getTimeStyleConverter(Locale.US, -1)", "Style '--' is invalid", e.getMessage());
        }
    }
    ...
    public void testNullDataForConverters(){
        try{
            ...
            assertEquals("# Incorrect exception message for api getDateTimeStyleConverter(null, -0, -0)", "Both dateStyle and timeStyle are invalid", e.getMessage());
        }
        try{
            DateServiceImpl.get().getDateStyleConverter(null, -0);
        }
        catch(IllegalArgumentException e){
            assertEquals("# Incorrect exception message for api getDateStyleConverter(null, -0)", "Style '--' is invalid", e.getMessage());
        }
        try{
            DateServiceImpl.get().getTimeStyleConverter(null, -0);
        }
        catch(IllegalArgumentException e){
            assertEquals("# Incorrect exception message for api getTimeStyleConverter(null, -0)", "Style '--' is invalid", e.getMessage());
        }
        try{
            DateServiceImpl.get().getPatternConverter(null, null);
        }
        catch(IllegalArgumentException e){
            assertEquals("# Incorrect exception message for api getPatternConverter(null, null)", "Invalid pattern specification", e.getMessage());
        }
        try{
    ...
    public void testNullDataForFormatAndParse(){
        try{
            ...
            assertEquals("# Incorrect exception message for api getDateTimeISO8601Converter().format(null)", "Date can not be null", e.getMessage());
        }
        try{
            DateServiceImpl.get().getDateTimeISO8601Converter().format(null, null);
        }
        catch(IllegalArgumentException e){
            assertEquals("# Incorrect exception message for api getDateTimeISO8601Converter().format(null, null)", "Date can not be null", e.getMessage());
        }
        try{
            DateServiceImpl.get().getDateTimeISO8601Converter().parse(null);
        }
        catch(IllegalArgumentException e){
            assertEquals("# Incorrect exception message for api getDateTimeISO8601Converter().parse(null)", "Date can not be null", e.getMessage());
        }
        try{
            DateServiceImpl.get().getDateTimeISO8601Converter().parse(null, null);
        }
        catch(IllegalArgumentException e){
            assertEquals("# Incorrect exception message for api getDateTimeISO8601Converter().parse(null, null)", "Date can not be null", e.getMessage());
        }
    }
    ...
    public void testParseWithTimeZone() {
        ...
        // but when they hit 1970-01-01, they're 5 hours later than when GMT folks hit it
        ...
        // but when they hit 1970-01-01, they're 8 hours earlier than when GMT folks hit it
        ...
        // If parse, without timezone is called, the JDK's default timezone should be used.
        ...
        // timezone is in this date - EST again - so we should get the same as above
