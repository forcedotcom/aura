...
public final class PerfMetricsComparatorTest extends UnitTestCase {
    ...
    public void testCompare() throws Exception {
        ...
        expected.setMetric(new PerfMetric("{name: \"metric1\", value: 10}"));
        expected.setMetric(new PerfMetric("{name: \"metric2\", value: 10}"));
        ...
        actual.setMetric(new PerfMetric("{name: \"metric1\", value: 10}"));
        actual.setMetric(new PerfMetric("{name: \"metric2\", value: 10}"));
        message = comparator.compare(expected, actual);
        assertNull(message, message);

        // UC: no message if within bounds
        actual = new PerfMetrics();
        actual.setMetric(new PerfMetric("{name: \"metric1\", value: 9}"));
        actual.setMetric(new PerfMetric("{name: \"metric2\", value: 11}"));
        message = comparator.compare(expected, actual);
        assertNull(message, message);

        // UC: message if not within bounds
        actual = new PerfMetrics();
        actual.setMetric(new PerfMetric("{name: \"metric1\", value: 5}"));
        actual.setMetric(new PerfMetric("{name: \"metric2\", value: 10}"));
        message = comparator.compare(expected, actual);
        ...
        expected.setMetric(new PerfMetric("{name: \"metric1\", value: 3}"));
        actual = new PerfMetrics();
        actual.setMetric(new PerfMetric("{name: \"metric1\", value: 2}"));
        message = comparator.compare(expected, actual);
        assertNull(message, message);

        // UC: show non-sorted sequence for MedianPerfMetrics
        expected = new PerfMetrics();
        expected.setMetric(new PerfMetric("{name: \"bytes\", value: 0}"));
