...
/**
 * Verify implementation of TypeUtil used to convert data from a given type to desired type.
 *
 *
 * @since 0.0.248
 */
public class TypeUtilTest extends UnitTestCase {
    ...
    public void testRegisteringTypeConverter(){
        ...
        assertEquals("Custom array type converter failed to convert.", new CustomPairType("lat",12890), arrayOfValues[0]);
        assertEquals("Custom array type converter failed to convert.", new CustomPairType("long",5467), arrayOfValues[1]);
    }
    /**
     * W-1295660
     * This is a big hole. Anybody can override the converters we have written and screw up the system.
     * Or may be this is acceptable.
     * Currently we use the last converter to be encountered while going through classes in classpath.
     */
    public void testRegisteringDuplicateTypeConverters(){
        assertTrue("Failed to register custom converter", TypeUtil.hasConverter(String.class, CustomDupType.class, null));
        try{
    ...
    public void testImplicitConversionThroughInheritance(){
        ...
        //Make sure conversion up the inheritance hierarchy did not go through any special procedure
    ...
    /**
     * Verify that TypeUtil doesn't barf because of bad converters.
     * Enable converter registration in TestTypeConvertersConfig.java
     */
