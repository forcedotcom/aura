...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xslt="http://xml.apache.org/xslt" version="1.0">
  ...
  <xsl:param name="jstestPrefix" select="'ComponentJSTest.'"/>
  <xsl:template match="node()|@*">
    <xsl:copy>
      <xsl:apply-templates select="node()|@*" />
    </xsl:copy>
  </xsl:template>
  <xsl:template match="//skippedtestcase[@classname='org.auraframework.test.ComponentJSTest$ComponentTestCase']">
    <xsl:copy>
  ...
  <xsl:template match="//testcase[@classname='org.auraframework.test.ComponentJSTest$ComponentTestCase']">
    <xsl:copy>
      <xsl:attribute name="classname">
        <xsl:value-of select="concat($jstestPrefix, substring-before(@name, '$'))" />
      </xsl:attribute>  
