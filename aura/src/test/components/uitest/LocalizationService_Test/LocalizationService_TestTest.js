...
({
	...
	testIsSame:{
    	test:function(component){
            var testCmp = component.find('myOutputDateTimeComp');
            aura.test.assertNotNull(testCmp);
            aura.test.addWaitFor(true, function(){return $A.test.getText(testCmp.find('span').getElement()).length > 0;},
                function(){
                    var outputDateStr = $A.test.getText(testCmp.find('span').getElement());
                    var dateObj1 = $A.localizationService.parseDateTime(outputDateStr, 'MMM DD, YYYY h:mm:ss A', 'en');
    ...
    testFormatDateUTC:{
        test:function(component){
            ...
            aura.test.addWaitFor(true, function(){return $A.test.getText(testCmp.find('span').getElement()).length > 0;},
                function() {
                    var outputDateStr = $A.test.getText(testCmp.find('span').getElement());
                    aura.test.assertEquals($A.localizationService.formatDateUTC('Sep 23, 2004', 'MMM DD, YYYY', 'en'),
                        'Sep 23, 2004', "formatDateUTC should keep the date constant");
                    aura.test.assertEquals($A.localizationService.formatDateUTC('Sep 23, 2004', 'MMM DD, YYYY', 'en'),
                         outputDateStr, "date should be the same as Fixed format UTC.");
                    aura.test.assertEquals($A.localizationService.formatDateUTC('Sep 23, 2004', '', 'en'), outputDateStr,
        ...
        test:function(component){
            ...
            aura.test.addWaitFor(true, function(){return $A.test.getText(testCmp.find('span').getElement()).length > 0;},
                function(){
                    ...
                    var dt = $A.localizationService.formatDateTimeUTC(dateObj, 'MMM DD, YYYY h:mm:ss A', 'en');
                    aura.test.assertEquals(dt, outputDateStr, "Both dates should be same.");
                });
        }
    },
