...
({
	...
	/**
	 * Verify the navigation indicators are displayed correctly when isDefault
	 * page is set.
	 */
	testDefaultPageNavigationIndicators : {
		...
		test : function(cmp) {
			var expectedDefaultPage = 2;
			var carousel = cmp.find("carousel1");
			this.assertNavigationIndicatorSelected(carousel,
					expectedDefaultPage);
		}
	},
	...
	testCarouselDefaultPageOverridesPageSetting : {
		...
		test : function(cmp) {
			...
			$A.test.assertFalse($A.util.isUndefinedOrNull(outputCmp),
					"Could not find output component");
	...
	testRerenderCarousel : {
		...
		test : [
				...
				function(cmp) {
					...
					$A.test.addWaitFor(true, function() {
						return waitForPageChange.apply(that, [ cmp,
								"carousel2", expectedPage.getLocalId() ]);
					});
					var indicators = this.getNavigationIndicators(carousel);
					$A.test
							.assertEquals(10, indicators.length,
	...
	testMaxPages : {
		...
		test : function(cmp) {
			...
			var indicators = this.getNavigationIndicators(carousel);
			$A.test
					.assertEquals(100, indicators.length,
							"Carousel does not have the correct number of navigation indicators");
		}
	},
	...
	testContinousFlowAriaValues : {
		...
		test : function(cmp) {
			...
			for ( var i = 0; i < pages.length; i++) {
				this.assertAriaExpandedNotDefined(pages[i].getElement());
			}
		}
	},
	...
	testCarouselWithinCarousel : {
		...
		test : function(cmp) {
			...
			$A.test.assertFalse($A.util.isUndefinedOrNull(parent),
					"Did not find parent carousel");
			var child1 = cmp.find("childCarousel-smaller");
			$A.test.assertFalse($A.util.isUndefinedOrNull(child1),
					"Did not find child carousel 1");
			var child2 = cmp.find("childCarousel-sameDim");
			$A.test.assertFalse($A.util.isUndefinedOrNull(child2),
					"Did not find child carousel 2");
			var child3 = cmp.find("childCarousel-larger");
			$A.test.assertFalse($A.util.isUndefinedOrNull(child3),
					"Did not find child carousel 3");
		}
	},

	/**
	 * With a carousel inside another carousel able to switch pages on a child
	 * carousel. All data and navigation indicators on the parent and child
	...
	testCarouselWithinCarouselSwitchChildPage : {
		...
		test : [
				function(cmp) {
					...
					var child = this
							.carouselInCarouselGetChildCarousel(parentPage);

					// go to page 2 on child carousel
					this.goToPageOnCarousel(cmp, "childCarousel-smaller", 2,
							"childCarousel-smaller-p2");
				},
				function(cmp) {
					...
					$A.test.assertEquals(childPage.getLocalId(),
							"childCarousel-smaller-p2",
	...
	/**
	 * With a carousel inside another carousel able to switch pages on the
	 * parent carousel. All data and navigation indicators on the parent and
	...
	testCarouselWithinCarouselSwitchParentPage : {
		...
		test : [
				function(cmp) {
					var parent = cmp.find("carouselInCarousel");
					this.goToPageOnCarousel(cmp, "carouselInCarousel", 2,
							"cinc-p2");
				},
				function(cmp) {
					...
					$A.test.assertEquals("cinc-p2", id,
							"Expected to switch to page 2 on parent carousel");
	...
	/**
	 * Test page change action is called when action is provided and page is
	 * changed
	 */
	...
	/**
	 * Goes through all pages and verify only one page is set to selected. Use
	 * getPageSelected() and/or assertPageSelected to verify a single page.
	 */
	...
	assertAriaExpanded : function(element, value) {
		$A.test.assertEquals(value.toString(), $A.test
				.getElementAttributeValue(element, "aria-expanded"),
				"aria-expanded value was incorrect");
	},

	assertAriaExpandedNotDefined : function(element) {
		$A.test.assertUndefinedOrNull(element.getAttribute("aria-expanded"),
				"aria-expanded should NOT defined.");
	},
	...
	assertNavigationIndicatorSelected : function(carousel, selectedIndicator) {
		...
		for ( var i = 0; i < indicators.length; i++) {
			...
			if ((selectedIndicator - 1) === i) {
				...
				this.assertAriaControlDefined(indicatorElem, true, errorMsg(
						selectedIndicator, (i + 1), "Checking aria-control"));
				this.assertAriaSelected(indicatorElem, true, errorMsg(
	...
	/**
	 * Verify the selected page is the correct MDM page. MDM pages have 6
	 * carouselPageItems. In a matirx:
	 * --------------------------------------------- [0] | New Post # | Task # |
	 * [1] --------------------------------------------- [2] | New Opportunity # |
	 * Thank Someone # | [3] --------------------------------------------- [4] |
	 * File # | Post a New Poll # | [5]
	 * ---------------------------------------------
	 */
	verifyMdmPage : function(page, pageItemNumber, expectedPageItemText) {
		...
		$A.test.assertEquals(expectedPageItemText, actualText,
				"Contents of MDM page incorrect.");
	},
	...
	waitForPageChange : function(cmp, carouselName, pageId) {
		...
		var selectedPageId = $A.util.isUndefinedOrNull(page) ? "" : page
				.getLocalId();
