...
({
	...
	testInitialOptionSelectedDOM : {
		test : function(cmp) {
			var inputSelectCmp = cmp.find("dynamicSelect");
            var options = inputSelectCmp.getElement().options;
            this.verifySelectedOption(options, "Option2", this.optionDOMSelected);
		}
	},
	...
	testChangeSelectionDynamic : {
		test : function(cmp) {
            ...
            var options = inputSelectCmp.getElement().options;
            this.verifySelectedOption(options, "Option4", this.optionDOMSelected);
		}
	},
	...
	verifySelectedOption : function(options, selectedLabel, predicate) {
		...
		if (!$A.util.isUndefinedOrNull(selectedOption)) {
			selectedOptionLabel = $A.util.isComponent(selectedOption)?selectedOption.get("v.label"):selectedOption.label;
		}
