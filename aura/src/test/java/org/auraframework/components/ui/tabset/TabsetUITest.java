...
public class TabsetUITest extends WebDriverTestCase {
    ...
    private By ACTIVE_LI_LOCATOR = By.cssSelector("li[class*='tabItem uiTabItem active'] > a");
    private By ACTIVE_SECTION = By.cssSelector("section[class*='tabBody uiTab active']");
 ...
 /**********************************************HELPER FUNCTIONS******************************************/   
    ...
    private void iterateThroughTabs(CharSequence rightOrDownArrow, CharSequence leftOrUpArrow){
        ...
        for(int i=0; i<TITLE_ARRAY.length; i++){
            ...
            assertEquals("The aria-controls id and section id do not match", element.getAttribute("aria-controls"), activeSection.getAttribute("id"));
            
            //Verify Body text matches what we think it should be
            assertTrue("The body of the section is not what it should be", activeSection.getText().contains(BODY_ARRAY[i]));
    ...
    /**
     * Making sure that when an element closes the element that is focused is no longer focused, and focus is on the next item
     * 
    ...
    /**
     * Function that will create a url based on the item attribute. Depending on what ITem is the page will render differently
    ...
    private void verifyElementIsActive(By loc){
    	...
    	assertTrue("The Active class name was not found in the non deleted element", el.getAttribute("class").contains("active"));
    }
    ...
    public void verifyElementFocus(String itemToVerifyAgainst){
        //Verify correct element is focused (verified with with the class that we are expecting the element to contain)
    	  String activeElementClass = (String) auraUITestingUtil.getEval("return $A.test.getActiveElement().getAttribute('class')");
    ...
    public void testNestedTabsDelete () throws MalformedURLException, URISyntaxException {
    	...
    	assertEquals("Size of the part tabBar was not as expected. Something must have been deleted", 2, elements.size());
    }  
     ...
     * 
     * Disabled against mobile since tabbing does not make sense on mobile
     * Tabbing with Safari acts oddly. For some strange reason, I have to grab the element I 
     * want and then send the tab key to put it into focus other wise nothing happens
     * 
     * @throws MalformedURLException
     * @throws URISyntaxException
     */

    @ExcludeBrowsers({BrowserType.SAFARI, BrowserType.ANDROID_PHONE, BrowserType.ANDROID_TABLET, BrowserType.IPHONE, BrowserType.IPAD})
