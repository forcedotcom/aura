 ...
 * AppCache only works for WebKit browsers.
 * 
 * @since 0.0.224
 */
...
public class AppCacheResourcesUITest extends WebDriverTestCase {
	...
	public void perBrowserSetUp() {
		...
		/*
		 * chrome on saucelab seems to be ok without deleting cookies, will
		 * verify this against jekins WebDriver driver = getDriver();
		 * driver.get("/auraFW/resources/aura/s.gif");//this won't work with
		 * driver, it doesn't know where /auraFW/.. is, we can use open("...")
		 * instead driver.manage().deleteAllCookies();//
		 */
	}
	...
	public void setUp() throws Exception {
		...
		createDef(StyleDef.class, String.format("%s://%s.%s",
	...
	public void runTestNoChanges(BrowserType bt) throws Exception {
		List<Request> logs = loadMonitorAndValidateApp(TOKEN, TOKEN, TOKEN,
				TOKEN);
		if (bt == BrowserType.SAFARI || bt == BrowserType.IPHONE
				|| bt == BrowserType.IPAD) {
	...
	public void runTestCacheError(BrowserType bt) throws Exception {
		List<Request> logs = loadMonitorAndValidateApp(TOKEN, TOKEN, TOKEN,
				TOKEN);
		if (bt == BrowserType.SAFARI || bt == BrowserType.IPAD
				|| bt == BrowserType.IPHONE) {
			assertRequests(getExpectedInitialRequestsSAFARI(), logs);
		} else if (bt == BrowserType.GOOGLECHROME) {
			assertRequests(getExpectedInitialRequestsGOOGLECHROME(), logs);
		} else {
		...
		expectedChange.add(new Request("/auraResource", null, null, "manifest",
				404)); // reset
		expectedChange.add(new Request("/aura", namespace + ":" + appName,
				null, "HTML", 302)); // hard refresh
		if (bt == BrowserType.GOOGLECHROME) {
			expectedChange.add(new Request(3, "/auraResource", null, null,
					"manifest", 200));
			expectedChange.add(new Request(2, "/aura", namespace + ":"
					+ appName, null, "HTML", 200));
		} else if (bt == BrowserType.SAFARI || bt == BrowserType.IPAD
				|| bt == BrowserType.IPHONE) {
			expectedChange.add(new Request("/auraResource", null, null,
					"manifest", 200));
			expectedChange.add(new Request("/aura", namespace + ":" + appName,
					null, "HTML", 200));
			expectedChange.add(new Request("/auraResource", null, null, "css",
					200));
			expectedChange.add(new Request("/auraResource", null, null, "js",
					200));
		} else
			fail("we only expect GOOGLECHROME or SAFARI for runTestCacheError");
		...
		assertFalse("Manifest cookie was not changed " + cookie.getValue(),
				"error".equals(cookie.getValue()));
	}
	...
	/**
	 * for Chrome and Safari/IPAD/IPHONE Opening uncached app that had a prior
	 * cache error will have limited caching
	 */
	...
	public void testCacheErrorWithEmptyCache() throws Exception {
		...
		expectedChange.add(new Request("/aura", namespace + ":" + appName,
				null, "HTML", 200));
		expectedChange
				.add(new Request("/auraResource", null, null, "css", 200));
	...
	/**
	 * Manifest request limit exceeded for the time period should result in
	 * reset.
	 */
	...
	/**
	 * for Chrome Manifest request limit exceeded for the time period should
	 * result in reset.
	 */
	@TargetBrowsers(BrowserType.GOOGLECHROME)
	...
	/**
	 * for SAFARI Manifest request limit exceeded for the time period should
	 * result in reset.
	 */
	@TargetBrowsers({ BrowserType.SAFARI })
	...
	/**
	 * for IPAD Manifest request limit exceeded for the time period should
	 * result in reset.
	 */
	@TargetBrowsers({ BrowserType.IPAD })
	...
	/**
	 * for IPHONE Manifest request limit exceeded for the time period should
	 * result in reset.
	 */
	@TargetBrowsers({ BrowserType.IPHONE })
	...
	/**
	 * for Chrome Opening cached app after namespace style change will trigger
	 * cache update.
	 */
	@TargetBrowsers({ BrowserType.GOOGLECHROME })
	...
	/**
	 * for Safari Opening cached app after namespace style change will trigger
	 * cache update.
	 */
	@TargetBrowsers({ BrowserType.SAFARI })
	...
	/**
	 * for IPAD Opening cached app after namespace style change will trigger
	 * cache update.
	 */
	@TargetBrowsers({ BrowserType.IPAD })
	...
	/**
	 * for IPHONE Opening cached app after namespace style change will trigger
	 * cache update.
	 */
	@TargetBrowsers({ BrowserType.IPHONE })
	...
	public void runTestComponentJsChange(BrowserType bt) throws Exception {
		...
		for (DefDescriptor<?> cd : getTargetComponent()
	...
	public void testFrameworkJsChange() throws Exception {
		...
		String replacement = getName() + System.currentTimeMillis();
		DirectiveBasedJavascriptGroup jsGroup = new DirectiveBasedJavascriptGroup(
				AURA, auraTestingUtil.getAuraJavascriptSourceDirectory(),
				"aura.test/Test.js", DirectiveTypes.DEFAULT_TYPES, EnumSet.of(
		...
		try {
			...
			Aura.getSourceControlAdapter().writeIfDifferent(
					new StringBuilder(originalContent.replace(TOKEN,
							replacement)), testJs);
			...
			logs = loadMonitorAndValidateApp(TOKEN, TOKEN, TOKEN, replacement);
			List<Request> expected = Lists.newArrayList(new Request(
					"/auraResource", null, null, "manifest", 200), new Request(
					"/aura", namespace + ":" + appName, null, null, 200));
			assertRequests(expected, logs);
			assertAppCacheStatus(Status.IDLE);
		} finally {
			if (testJs != null) {
				Aura.getSourceControlAdapter().writeIfDifferent(
						new StringBuilder(originalContent), testJs);
				testJs.setLastModified(System.currentTimeMillis());
			}
		}
	}
	...
	private <T extends Definition> DefDescriptor<T> createDef(
		...
		DefDescriptor<T> desc = Aura.getDefinitionService().getDefDescriptor(
				qualifiedName, defClass);
		addSourceAutoCleanup(desc, content);
		return desc;
	}

	private String getManifestCookieName() {
		return String.format(COOKIE_NAME, getAuraModeForCurrentBrowser()
				.toString(), namespace, appName);
	}

	private void assertAppCacheStatus(Status status) {
		Status actual = Status.values()[Integer.parseInt(auraUITestingUtil
				.getEval("return window.applicationCache.status;").toString())];
	...
	/**
	 * this function will check each request in actual list against expected
	 * list. fudge is the number this request suppose to show up. we remove the
	 * request from expected list once it has been visited #fudge times. any
	 * missing request will be added to missingRequests list.
	 * 
	 * @param expected
	 *            : list of expected request
	 * @param actual
	 *            : list of actual request captured by log
	 * @throws Exception
	 */
	...
	/**
	 * Load and get all the log lines for the app load. Some sanity checks that
	 * our simple test app is functional after cache resolutions.
	 * <ul>
	...
	private List<Request> endMonitoring() {
		...
		for (Map<String, Object> log : TestLoggingAdapterController
			...
			if (!"GET".equals(log.get("requestMethod"))) {
				//
				// Log ignored lines so that we can monitor what happens. The
				// line above
	...
	/**
	 * For Safari Get the set of expected requests on change. These are the
	 * requests that we expect for filling the app cache. The explanation is as
	 * follows.
	 * <ul>
	 ...
	 * <ul>
	 * The primary difference between this and the initial requests is that we
	 * get the initial page twice
	 * 
	...
	private List<Request> getExpectedChangeRequestsSAFARI() {
		return ImmutableList.of(
				new Request("/aura", namespace + ":" + appName, null, "HTML",
						302), // hard refresh
				new Request("/auraResource", null, null, "manifest", 404), // manifest
																			// out
																			// of
																			// date
				new Request("/auraResource", null, null, "manifest", 200),
				new Request(2, "/aura", namespace + ":" + appName, null,
						"HTML", 200), // rest are cache updates
				new Request(2, "/auraResource", null, null, "css", 200),
				new Request(2, "/auraResource", null, null, "js", 200));
	}

	/**
	 * For Chrome Get the set of expected requests on change. These are the
	 * requests that we expect for filling the app cache. The explanation is as
	 * follows.
	 * <ul>
	 * <li>The manifest is pulled</li>
	 ...
	 * <ul>
	 * The primary difference between this and the initial requests is that we
	 * don't get the initial page twice, and we get the manifest three times...
	 * odd that.
	 * 
	 * @return the list of request objects, not necessarily in order.
	 */
	...
	private List<Request> getExpectedInitialRequestsSAFARI() {
		return ImmutableList.of(new Request(1, "/aura", namespace + ":"
				+ appName, null, "HTML", 200), new Request(1, "/auraResource",
				null, null, "manifest", 200), new Request(2, "/auraResource",
				null, null, "css", 200), new Request(2, "/auraResource", null,
				null, "js", 200));
	}

	/**
	 * For Chrome Get the set of expected initial requests. These are the
	 * requests that we expect for filling the app cache. The explanation is as
	 * follows.
	 * <ul>
	 ...
	 * <li>Finally, the browser re-fetches the manifest to check contents</li>
	 * <ul>
	 * Note that there are two requests for the initial page, one as the first
	 * request, and one to fill the app cache (odd, but true). There are also
	 * two manifest requests.
	 * 
	 * @return the list of request objects, not necessarily in order.
	 */
	private List<Request> getExpectedInitialRequestsGOOGLECHROME() {
		return ImmutableList.of(new Request(2, "/aura", namespace + ":"
				+ appName, null, "HTML", 200), new Request(2, "/auraResource",
				null, null, "manifest", 200), new Request("/auraResource",
				null, null, "css", 200), new Request("/auraResource", null,
				null, "js", 200));
	}

	/**
	 * A request object, which can either be an 'expected' request, or an
	 * 'actual' request. Expected requests can also have a fudge factor allowing
	 * multiple requests for the resource. This is very helpful for different
	 * browsers doing diferent things with the manifest. We allow multiple
	 * fetches of both the manifest and initial page in both the initial request
	...
	static class Request extends HashMap<String, String> {
		...
		private static final ImmutableSet<String> validKeys = ImmutableSet.of(
				"URI", "tag", "namespaces", "format", "httpStatus");
		 ...
		 * 
		 * @return true if we got the request. each request from expected list
		 *         must show up at least once in the actual list.
		 */
		 ...
		 * 
		 * @return true if it should be removed.count > fudge: browsers don't
		 *         behave consistently. better have a loose bound here. we are
		 *         comparing two requests list: actual list and expected list.
		 *         count start at 0, we are expecting 1,2,..,fudge, or fudge+1
		 *         request. once we have some request X that show up fudge+1 in
		 *         actual list, X get removed from expected list. then if we
		 *         receive another X again, it will be added to unexpected
		 *         requestes list and error out.
		 */
		public boolean mark() {
			if (fudge == 0) {
				return true;
