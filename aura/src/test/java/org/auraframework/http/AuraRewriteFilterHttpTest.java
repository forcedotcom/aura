...
public class AuraRewriteFilterHttpTest extends AuraHttpTestCase {
    ...
    public void testDoFilterOfValidURLs()throws Exception{
        ...
        assertUrlResponse("Didn't forward url with namespace and application name.",
                "/test/fakeApplication.app", HttpStatus.SC_OK);
    ...
    public void _testDoFilterOfMalformedURLs()throws Exception{
        ...
        assertUrlResponse("Shouldn't forward url without component name.",
                "/aura/.cmp?aura.mode=PROD", HttpStatus.SC_NOT_FOUND);
        ...
        assertUrlResponse("Shouldn't forward url without namespace.", "/text.cmp", HttpStatus.SC_NOT_FOUND);
        assertUrlResponse("Shouldn't forward url while posting directly to servlet without component name.",
                "/aura/", HttpStatus.SC_NOT_FOUND);
        assertUrlResponse("Shouldn't forward url while posting directly to aura servlet.",
                "/aura", HttpStatus.SC_NOT_FOUND);
        assertUrlResponse("Shouldn't forward url with double suffix", "/aura/text.cmp.cmp", HttpStatus.SC_NOT_FOUND);
        assertUrlResponse("Shouldn't forward url with double mixed suffix", "/aura/text.cmp.app", HttpStatus.SC_NOT_FOUND);
        ...
        assertUrlResponse("Shouldn't forward url with * for namespace and component name", "/*/*.cmp", HttpStatus.SC_NOT_FOUND);
    }
}
