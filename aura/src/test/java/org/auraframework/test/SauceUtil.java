...
public final class SauceUtil {
    ...
    private static final String SAUCE_ACCESS_KEY = System.getProperty("sauce.access.key", "4df61bdf-1696-4f73-b869-ead4c7603dca");
    private static final String SAUCE_WEB_DRIVER_URL = "http://"
            + SAUCE_USERNAME + ':' + SAUCE_ACCESS_KEY
            + "@saucelabs.com:4444/wd/hub";
    private static final String SAUCE_CONNECT_URL = "http://" + SAUCE_USERNAME
            + ':' + SAUCE_ACCESS_KEY
    ...
    public static boolean areTestsRunningOnSauce() {
        return "saucelabs.com".equals(System
                .getProperty(WebDriverProvider.WEBDRIVER_SERVER_PROPERTY));
    }

    public static URL getSauceServerUrl() throws MalformedURLException {
        String hubURL = TUNNEL_SELENIUM_COMMANDS_THROUGH_SAUCE_CONNECT ? SAUCE_CONNECT_URL
                : SAUCE_WEB_DRIVER_URL;
     ...
     * @param test
     * @param browserType
     *            String identifying browser and version (same String as the one
     *            used by SeleniumTest.BrowserType)
     * @return Capabilities for the browserType as required by SauceLabs
     */
    public static DesiredCapabilities getCapabilities(BrowserType browserType,
            TestCase test) {
        ...
        // adding timeouts to prevent jobs to run for too long when problems occur:
        // see http://saucelabs.com/docs/ondemand/additional-config#timeouts
        capabilities.setCapability("max-duration", 300); // 5min, default was 30mins
        capabilities.setCapability("command-timeout", 60); // 1min, default was 5mins
        capabilities.setCapability("idle-timeout", 60); // 1min, default was 90secs
    ...
    public static String getLinkToPublicJobInSauce(WebDriver driver) {
        ...
        String sessionId = ((RemoteWebDriver) driver).getSessionId().toString();
        try {
            return "https://saucelabs.com/jobs/" + sessionId + "?auth="
                    + authToken(sessionId);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    private static String authToken(String jobId) throws Exception {
        ...
        SecretKeySpec secretKey = new SecretKeySpec(message.getBytes(),
                "HMACMD5");
