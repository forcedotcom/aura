...
public class AbstractActionImplTest {
    ...
    private static class MyAction extends AbstractActionImpl<ActionDef> {
        public MyAction(DefDescriptor<ControllerDef> controllerDescriptor, ActionDef actionDef,
            ...
            this(mock(ConfigAdapter.class), controllerDescriptor, actionDef, paramValues);
        }
        ...
        public void run() throws AuraExecutionException {
            // Test class and this method does not actually need to do anything because the test can validate
            // 
        }
    ...
    @Test
    public void testId() {
        ActionDef def = mock(ActionDef.class);
    ...
    private static Action getActionWithId(String id) {
        ActionDef def = mock(ActionDef.class);
        Action test = new MyAction(null, def, null);
        test.setId(id);
        return test;
    ...
    public void testStorable() {
        ...
        assertThat("isStorable should be initialized to false", Boolean.valueOf(test.isStorable()), equalTo(FALSE));
        test.setStorable();
        assertThat("isStorable should change on setStorable", Boolean.valueOf(test.isStorable()), equalTo(TRUE));
        test.setStorable();
        assertThat("isStorable should not change on second setStorable", Boolean.valueOf(test.isStorable()), equalTo(TRUE));
    }
    ...
    public void testOfflineAction() {
        ...
        Assert.assertEquals("isOfflineAction should be initialized to false", FALSE, Boolean.valueOf(test.isOfflineAction()));
        test.markOfflineAction();
        Assert.assertEquals("isOfflineAction should change on markOfflineAction", TRUE, Boolean.valueOf(test.isOfflineAction()));
        Assert.assertEquals("id should change on markOfflineAction", "s", test.getId());
        test.setId("x");
        test.markOfflineAction();
        Assert.assertEquals("isOfflineAction should not change on second markOfflineAction", TRUE, Boolean.valueOf(test.isOfflineAction()));
    ...
    public void testDescriptor() {
        ActionDef def = mock(ActionDef.class);
        Action test = new MyAction(null, def, null);
        DefDescriptor<ActionDef> expectedDesc = mock(DefDescriptor.class);
        when(def.getDescriptor()).thenReturn(expectedDesc);
    ...
    public void testParams() {
        ...
        LoggingContext.KeyValueLogger logger = mock(LoggingContext.KeyValueLogger.class);
        ...
        when(def.getLoggableParams()).thenReturn(Lists.newArrayList("a", "b"));
        test.logParams(logger);
        verify(logger, times(1)).log("a", "b");
        verify(logger, times(1)).log("b", "null");
        verifyNoMoreInteractions(logger);

        test = new MyAction(null, def, null);
        // Rebuild logger to check the logging correctly.
        logger = mock(LoggingContext.KeyValueLogger.class);
        Assert.assertEquals("null params results in empty params", 0, test.getParams().size());
        test.logParams(logger);
        verify(logger, times(1)).log("a", "null");
        verify(logger, times(1)).log("b", "null");
        verifyNoMoreInteractions(logger);
    }
